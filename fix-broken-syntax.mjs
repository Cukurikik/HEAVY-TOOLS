...
const rootDirs = [
  "src/app/modules/audio",
  "src/app/modules/video",
  "src/app/modules/converter",
  "src/app/modules/pdf",
];
...
for (const rootDir of rootDirs) {
  walk(rootDir, (filepath) => {
    if (filepath.endsWith(".ts")) {
      ...
      // 4. Remove unused vars completely
      ...
      if (filepath.endsWith("audio/04-converter/converter.worker.ts")) {
        if (content.includes("_targetFormat")) {
          content = content.replace(
            /const\s*\{\s*inputData\s*,\s*_targetFormat\s*\}\s*=\s*_event\.data\.config;/,
      ...
      if (
        filepath.endsWith("audio/15-analyser/analyser.worker.ts") ||
        filepath.endsWith("audio/28-transcriber/transcriber.worker.ts")
      ) {
        if (content.includes("(_event: MessageEvent)")) {
          content = content.replace(
            /addEventListener\('message',\s*\(_event:\s*MessageEvent\)\s*=>\s*\{/,
      ...
      if (filepath.endsWith("audio/shared/engine/onnx-audio.service.ts")) {
        if (content.includes("_modelName: string")) {
          content = content.replace(
            /async\s+loadModel\(modelUrl:\s*string,\s*_modelName:\s*string\)/,
      ...
      if (filepath.endsWith("converter/shared/engine/libreoffice.service.ts")) {
        if (content.includes("_file: File")) {
          content = content.replace(
            /async\s+convert\(_file:\s*File,\s*_targetFormat:\s*string\)/,
            "async convert()",
          );
          changed = true;
        }
      ...
      if (filepath.endsWith("pdf/shared/engine/opfs.service.ts")) {
        if (content.includes("_path: string")) {
          content = content.replace(
            /async\s+exists\(_path:\s*string\)/,
            "async exists()",
          );
          changed = true;
        }
      ...
      if (
        content.includes("ExportFormat") &&
        !content.match(/\bexport type ExportFormat\b/) &&
        !content.match(/\bexport const ExportFormat\b/)
      ) {
      ...
      if (filepath.endsWith("pdf/shared/engine/opfs.service.ts")) {
        if (content.includes("_path: string")) {
          content = content.replace(
            /async\s+getFile\(_path:\s*string\)/,
            "async getFile()",
          );
          changed = true;
        }
