...
// src/app/modules/pdf/10-crop-resize/cropResize.store.ts
var init = {
  inputFile: null,
  pdfMeta: null,
  status: "idle",
...
var CropResizeActions = {
  ...
  loadMetaSuccess: createAction("[CropResize] Meta Success", props()),
  loadMetaFailure: createAction("[CropResize] Meta Failure", props()),
  startProcessing: createAction("[CropResize] Start Processing"),
  updateProgress: createAction("[CropResize] Update Progress", props()),
  processingSuccess: createAction("[CropResize] Processing Success", props()),
  processingFailure: createAction("[CropResize] Processing Failure", props()),
  resetState: createAction("[CropResize] Reset State")
};
var cropResizeFeature = createFeature({
  name: "cropResizePdf",
  reducer: createReducer(init, on(CropResizeActions.loadFile, (s, a) => __spreadProps(__spreadValues({}, s), { inputFile: a.file, status: "loading", outputBlob: null, errorMessage: null, progress: 0 })), on(CropResizeActions.loadMetaSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { pdfMeta: a.meta, status: "idle" })), on(CropResizeActions.loadMetaFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message })), on(CropResizeActions.startProcessing, (s) => __spreadProps(__spreadValues({}, s), { status: "processing", progress: 0 })), on(CropResizeActions.updateProgress, (s, a) => __spreadProps(__spreadValues({}, s), { progress: a.progress })), on(CropResizeActions.processingSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { status: "done", progress: 100, outputBlob: a.outputBlob, outputSizeMB: a.outputSizeMB })), on(CropResizeActions.processingFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message, retryable: a.retryable })), on(CropResizeActions.resetState, () => init))
});
var { selectCropResizePdfState, selectStatus, selectProgress, selectOutputBlob } = cropResizeFeature;
var selectCropResizeCanProcess = createSelector(selectCropResizePdfState, (s) => !!s.inputFile && s.status === "idle");
var selectCropResizeIsLoading = createSelector(selectStatus, (s) => s === "loading" || s === "processing" || s === "rendering");

// src/app/modules/pdf/10-crop-resize/cropResize.component.ts
function CropResizeComponent_Conditional_9_Conditional_16_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "div", 18);
    \u0275\u0275text(1, " Processing... ");
  }
}
function CropResizeComponent_Conditional_9_Conditional_18_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275text(0, " \u{1F4D0} Execute ");
  }
}
function CropResizeComponent_Conditional_9_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275pipe(14, "async");
    \u0275\u0275pipe(15, "async");
    \u0275\u0275listener("click", function CropResizeComponent_Conditional_9_Template_button_click_13_listener() {
      \u0275\u0275restoreView(_r1);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.onProcess());
    });
    \u0275\u0275conditionalCreate(16, CropResizeComponent_Conditional_9_Conditional_16_Template, 2, 0);
    \u0275\u0275pipe(17, "async");
    \u0275\u0275conditionalBranchCreate(18, CropResizeComponent_Conditional_9_Conditional_18_Template, 1, 0);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const meta_r3 = ctx;
...
function CropResizeComponent_Conditional_17_Template(rf, ctx) {
  if (rf & 1) {
...
function CropResizeComponent_Conditional_19_Template(rf, ctx) {
  if (rf & 1) {
...
var CropResizeComponent = class _CropResizeComponent {
  ...
  state$ = this.store.select(selectCropResizePdfState);
  ...
  async onFileSelected(files) {
    ...
    try {
      ...
      this.store.dispatch(CropResizeActions.loadMetaFailure({ errorCode: "FILE_CORRUPTED", message: "Could not read PDF structure." }));
    }
  }
  onProcess() {
    ...
    this.state$.subscribe((state) => {
      if (!state.inputFile)
        return;
      this.bridge.process(() => new Worker(new URL("worker-FUGWZFXF.js", import.meta.url), { type: "module" }), { file: state.inputFile }).subscribe({
        next: (msg) => {
          if (msg.type === "progress")
            this.store.dispatch(CropResizeActions.updateProgress({ progress: msg.value ?? 0 }));
          if (msg.type === "complete" && msg.data) {
            this.store.dispatch(CropResizeActions.processingSuccess({ outputBlob: msg.data, outputSizeMB: msg.data.size / 1048576 }));
          }
          if (msg.type === "error") {
            this.store.dispatch(CropResizeActions.processingFailure({ errorCode: msg.errorCode ?? "UNKNOWN_ERROR", message: msg.message ?? "Failed", retryable: true }));
          }
        }
      });
    }).unsubscribe();
  ...
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(CropResizeComponent, [{
    ...
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Crop margins or resize pages.</p>
      </header>
         ...
         <div class="space-y-4">
            ...
            @if ((state$ | async)?.status === 'done') {
                <app-pdf-export-panel 
                  ...
                  defaultFilename="omni_cropResize.pdf" 
                />
            }
         </div>
      </div>
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(CropResizeComponent, { className: "CropResizeComponent", filePath: "src/app/modules/pdf/10-crop-resize/cropresize.component.ts", lineNumber: 82 });
})();

// src/app/modules/pdf/10-crop-resize/cropResize.service.ts
var CropResizeService = class _CropResizeService {
  prepareConfig() {
    return {};
  }
  static \u0275fac = function CropResizeService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _CropResizeService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _CropResizeService, factory: _CropResizeService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(CropResizeService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();

// src/app/modules/pdf/10-crop-resize/cropResize.schema.ts
var CropResizeSchema = external_exports.object({
...
export {
  ...
  CropResizeService,
  cropResizeFeature,
  selectCropResizeCanProcess,
  selectCropResizeIsLoading,
  selectCropResizePdfState,
  selectOutputBlob,
  selectProgress,
  selectStatus
};
//# sourceMappingURL=chunk-2EH26KUO.mjs.map
