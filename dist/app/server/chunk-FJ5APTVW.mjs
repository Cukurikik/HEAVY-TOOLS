...
// src/app/modules/pdf/20-compare/compare.store.ts
var init = {
  inputFile: null,
  pdfMeta: null,
  status: "idle",
...
var CompareActions = {
  ...
  loadMetaSuccess: createAction("[Compare] Meta Success", props()),
  loadMetaFailure: createAction("[Compare] Meta Failure", props()),
  startProcessing: createAction("[Compare] Start Processing"),
  updateProgress: createAction("[Compare] Update Progress", props()),
  processingSuccess: createAction("[Compare] Processing Success", props()),
  processingFailure: createAction("[Compare] Processing Failure", props()),
  resetState: createAction("[Compare] Reset State")
};
var compareFeature = createFeature({
  name: "comparePdf",
  reducer: createReducer(init, on(CompareActions.loadFile, (s, a) => __spreadProps(__spreadValues({}, s), { inputFile: a.file, status: "loading", outputBlob: null, errorMessage: null, progress: 0 })), on(CompareActions.loadMetaSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { pdfMeta: a.meta, status: "idle" })), on(CompareActions.loadMetaFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message })), on(CompareActions.startProcessing, (s) => __spreadProps(__spreadValues({}, s), { status: "processing", progress: 0 })), on(CompareActions.updateProgress, (s, a) => __spreadProps(__spreadValues({}, s), { progress: a.progress })), on(CompareActions.processingSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { status: "done", progress: 100, outputBlob: a.outputBlob, outputSizeMB: a.outputSizeMB })), on(CompareActions.processingFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message, retryable: a.retryable })), on(CompareActions.resetState, () => init))
});
var { selectComparePdfState, selectStatus, selectProgress, selectOutputBlob } = compareFeature;
var selectCompareCanProcess = createSelector(selectComparePdfState, (s) => !!s.inputFile && s.status === "idle");
var selectCompareIsLoading = createSelector(selectStatus, (s) => s === "loading" || s === "processing" || s === "rendering");

// src/app/modules/pdf/20-compare/compare.component.ts
function CompareComponent_Conditional_9_Conditional_16_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "div", 18);
    \u0275\u0275text(1, " Processing... ");
  }
}
function CompareComponent_Conditional_9_Conditional_18_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275text(0, " \u2696\uFE0F Execute ");
  }
}
function CompareComponent_Conditional_9_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275pipe(14, "async");
    \u0275\u0275pipe(15, "async");
    \u0275\u0275listener("click", function CompareComponent_Conditional_9_Template_button_click_13_listener() {
      \u0275\u0275restoreView(_r1);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.onProcess());
    });
    \u0275\u0275conditionalCreate(16, CompareComponent_Conditional_9_Conditional_16_Template, 2, 0);
    \u0275\u0275pipe(17, "async");
    \u0275\u0275conditionalBranchCreate(18, CompareComponent_Conditional_9_Conditional_18_Template, 1, 0);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const meta_r3 = ctx;
...
function CompareComponent_Conditional_17_Template(rf, ctx) {
  if (rf & 1) {
...
var CompareComponent = class _CompareComponent {
  ...
  state$ = this.store.select(selectComparePdfState);
  ...
  async onFileSelected(files) {
    ...
    try {
      ...
      this.store.dispatch(CompareActions.loadMetaFailure({ errorCode: "FILE_CORRUPTED", message: "Could not read PDF structure." }));
    }
  }
  onProcess() {
    ...
    this.state$.subscribe((state) => {
      if (!state.inputFile)
        return;
      this.bridge.process(() => new Worker(new URL("worker-PFYQM72Q.js", import.meta.url), { type: "module" }), { file: state.inputFile }).subscribe({
        next: (msg) => {
          if (msg.type === "progress")
            this.store.dispatch(CompareActions.updateProgress({ progress: msg.value ?? 0 }));
          if (msg.type === "complete" && msg.data) {
            this.store.dispatch(CompareActions.processingSuccess({ outputBlob: msg.data, outputSizeMB: msg.data.size / 1048576 }));
          }
          if (msg.type === "error") {
            this.store.dispatch(CompareActions.processingFailure({ errorCode: msg.errorCode ?? "UNKNOWN_ERROR", message: msg.message ?? "Failed", retryable: true }));
          }
        }
      });
    }).unsubscribe();
  ...
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(CompareComponent, [{
    ...
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Find differences between PDFs.</p>
      </header>
         ...
         <div class="space-y-4">
            ...
            @if ((state$ | async)?.status === 'done') {
                <app-pdf-export-panel 
                  ...
                  defaultFilename="omni_compare.pdf" 
                />
            }
         </div>
      </div>
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(CompareComponent, { className: "CompareComponent", filePath: "src/app/modules/pdf/20-compare/compare.component.ts", lineNumber: 82 });
})();

// src/app/modules/pdf/20-compare/compare.service.ts
...
// src/app/modules/pdf/20-compare/compare.schema.ts
...
export {
  ...
  CompareService,
  compareFeature,
  selectCompareCanProcess,
  selectCompareIsLoading,
  selectComparePdfState,
  selectOutputBlob,
  selectProgress,
  selectStatus
};
//# sourceMappingURL=chunk-FJ5APTVW.mjs.map
