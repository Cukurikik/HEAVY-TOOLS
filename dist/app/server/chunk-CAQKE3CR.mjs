...
// src/app/modules/pdf/19-thumbnail-generator/thumbnailGenerator.store.ts
var init = {
  inputFile: null,
  pdfMeta: null,
  status: "idle",
...
var ThumbnailGeneratorActions = {
  ...
  loadMetaSuccess: createAction("[ThumbnailGenerator] Meta Success", props()),
  loadMetaFailure: createAction("[ThumbnailGenerator] Meta Failure", props()),
  startProcessing: createAction("[ThumbnailGenerator] Start Processing"),
  updateProgress: createAction("[ThumbnailGenerator] Update Progress", props()),
  processingSuccess: createAction("[ThumbnailGenerator] Processing Success", props()),
  processingFailure: createAction("[ThumbnailGenerator] Processing Failure", props()),
...
var thumbnailGeneratorFeature = createFeature({
  ...
  reducer: createReducer(init, on(ThumbnailGeneratorActions.loadFile, (s, a) => __spreadProps(__spreadValues({}, s), { inputFile: a.file, status: "loading", outputBlob: null, errorMessage: null, progress: 0 })), on(ThumbnailGeneratorActions.loadMetaSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { pdfMeta: a.meta, status: "idle" })), on(ThumbnailGeneratorActions.loadMetaFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message })), on(ThumbnailGeneratorActions.startProcessing, (s) => __spreadProps(__spreadValues({}, s), { status: "processing", progress: 0 })), on(ThumbnailGeneratorActions.updateProgress, (s, a) => __spreadProps(__spreadValues({}, s), { progress: a.progress })), on(ThumbnailGeneratorActions.processingSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { status: "done", progress: 100, outputBlob: a.outputBlob, outputSizeMB: a.outputSizeMB })), on(ThumbnailGeneratorActions.processingFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message, retryable: a.retryable })), on(ThumbnailGeneratorActions.resetState, () => init))
});
var { selectThumbnailGeneratorPdfState, selectStatus, selectProgress, selectOutputBlob } = thumbnailGeneratorFeature;
var selectThumbnailGeneratorCanProcess = createSelector(selectThumbnailGeneratorPdfState, (s) => !!s.inputFile && s.status === "idle");
var selectThumbnailGeneratorIsLoading = createSelector(selectStatus, (s) => s === "loading" || s === "processing" || s === "rendering");

// src/app/modules/pdf/19-thumbnail-generator/thumbnailGenerator.component.ts
function ThumbnailGeneratorComponent_Conditional_9_Conditional_16_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "div", 18);
    \u0275\u0275text(1, " Processing... ");
  }
}
function ThumbnailGeneratorComponent_Conditional_9_Conditional_18_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275text(0, " \u{1F4F8} Execute ");
  }
}
function ThumbnailGeneratorComponent_Conditional_9_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275pipe(14, "async");
    \u0275\u0275pipe(15, "async");
    \u0275\u0275listener("click", function ThumbnailGeneratorComponent_Conditional_9_Template_button_click_13_listener() {
      \u0275\u0275restoreView(_r1);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.onProcess());
    });
    \u0275\u0275conditionalCreate(16, ThumbnailGeneratorComponent_Conditional_9_Conditional_16_Template, 2, 0);
    \u0275\u0275pipe(17, "async");
    \u0275\u0275conditionalBranchCreate(18, ThumbnailGeneratorComponent_Conditional_9_Conditional_18_Template, 1, 0);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const meta_r3 = ctx;
...
function ThumbnailGeneratorComponent_Conditional_17_Template(rf, ctx) {
  if (rf & 1) {
...
function ThumbnailGeneratorComponent_Conditional_19_Template(rf, ctx) {
  if (rf & 1) {
...
var ThumbnailGeneratorComponent = class _ThumbnailGeneratorComponent {
  ...
  state$ = this.store.select(selectThumbnailGeneratorPdfState);
  ...
  async onFileSelected(files) {
    ...
    try {
      ...
      this.store.dispatch(ThumbnailGeneratorActions.loadMetaFailure({ errorCode: "FILE_CORRUPTED", message: "Could not read PDF structure." }));
    }
  }
  onProcess() {
    ...
    this.state$.subscribe((state) => {
      if (!state.inputFile)
        return;
      this.bridge.process(() => new Worker(new URL("worker-2DQKPETZ.js", import.meta.url), { type: "module" }), { file: state.inputFile }).subscribe({
        next: (msg) => {
          if (msg.type === "progress")
            this.store.dispatch(ThumbnailGeneratorActions.updateProgress({ progress: msg.value ?? 0 }));
          if (msg.type === "complete" && msg.data) {
            this.store.dispatch(ThumbnailGeneratorActions.processingSuccess({ outputBlob: msg.data, outputSizeMB: msg.data.size / 1048576 }));
          }
          if (msg.type === "error") {
            this.store.dispatch(ThumbnailGeneratorActions.processingFailure({ errorCode: msg.errorCode ?? "UNKNOWN_ERROR", message: msg.message ?? "Failed", retryable: true }));
          }
        }
      });
    }).unsubscribe();
    ...
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        <h1 class="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-rose-400 drop-shadow-lg">
          \u{1F4F8} Thumbnail Generator
        </h1>
        <p class="text-white/50 text-sm">Create thumbnail previews.</p>
      </header>
         ...
         <div class="space-y-4">
            ...
            @if ((state$ | async)?.status === 'done') {
                <app-pdf-export-panel 
                  ...
                  defaultFilename="omni_thumbnailGenerator.pdf" 
                />
            }
         </div>
      </div>
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ThumbnailGeneratorComponent, { className: "ThumbnailGeneratorComponent", filePath: "src/app/modules/pdf/19-thumbnail-generator/thumbnailgenerator.component.ts", lineNumber: 82 });
})();

// src/app/modules/pdf/19-thumbnail-generator/thumbnailGenerator.service.ts
var ThumbnailGeneratorService = class _ThumbnailGeneratorService {
  prepareConfig() {
    return {};
  }
  static \u0275fac = function ThumbnailGeneratorService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _ThumbnailGeneratorService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ThumbnailGeneratorService, factory: _ThumbnailGeneratorService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ThumbnailGeneratorService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();

// src/app/modules/pdf/19-thumbnail-generator/thumbnailGenerator.schema.ts
var ThumbnailGeneratorSchema = external_exports.object({
  inputFile: PdfFileSchema
});
export {
  ThumbnailGeneratorActions,
  ThumbnailGeneratorComponent,
  ThumbnailGeneratorSchema,
  ThumbnailGeneratorService,
  selectOutputBlob,
  selectProgress,
  selectStatus,
  selectThumbnailGeneratorCanProcess,
  selectThumbnailGeneratorIsLoading,
  selectThumbnailGeneratorPdfState,
  thumbnailGeneratorFeature
};
//# sourceMappingURL=chunk-CAQKE3CR.mjs.map
