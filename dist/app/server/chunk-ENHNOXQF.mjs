...
// src/app/modules/pdf/04-converter/converter.store.ts
var init = {
  inputFile: null,
  pdfMeta: null,
  status: "idle",
...
var ConverterActions = {
  ...
  loadMetaSuccess: createAction("[Converter] Meta Success", props()),
  loadMetaFailure: createAction("[Converter] Meta Failure", props()),
  startProcessing: createAction("[Converter] Start Processing"),
  updateProgress: createAction("[Converter] Update Progress", props()),
  processingSuccess: createAction("[Converter] Processing Success", props()),
  processingFailure: createAction("[Converter] Processing Failure", props()),
  resetState: createAction("[Converter] Reset State")
};
var converterFeature = createFeature({
  name: "converterPdf",
  reducer: createReducer(init, on(ConverterActions.loadFile, (s, a) => __spreadProps(__spreadValues({}, s), { inputFile: a.file, status: "loading", outputBlob: null, errorMessage: null, progress: 0 })), on(ConverterActions.loadMetaSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { pdfMeta: a.meta, status: "idle" })), on(ConverterActions.loadMetaFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message })), on(ConverterActions.startProcessing, (s) => __spreadProps(__spreadValues({}, s), { status: "processing", progress: 0 })), on(ConverterActions.updateProgress, (s, a) => __spreadProps(__spreadValues({}, s), { progress: a.progress })), on(ConverterActions.processingSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { status: "done", progress: 100, outputBlob: a.outputBlob, outputSizeMB: a.outputSizeMB })), on(ConverterActions.processingFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message, retryable: a.retryable })), on(ConverterActions.resetState, () => init))
});
var { selectConverterPdfState, selectStatus, selectProgress, selectOutputBlob } = converterFeature;
var selectConverterCanProcess = createSelector(selectConverterPdfState, (s) => !!s.inputFile && s.status === "idle");
var selectConverterIsLoading = createSelector(selectStatus, (s) => s === "loading" || s === "processing" || s === "rendering");

// src/app/modules/pdf/04-converter/converter.component.ts
function ConverterComponent_Conditional_9_Conditional_16_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "div", 18);
    \u0275\u0275text(1, " Processing... ");
  }
}
function ConverterComponent_Conditional_9_Conditional_18_Template(rf, ctx) {
  if (rf & 1) {
...
function ConverterComponent_Conditional_9_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275pipe(14, "async");
    \u0275\u0275pipe(15, "async");
    \u0275\u0275listener("click", function ConverterComponent_Conditional_9_Template_button_click_13_listener() {
      \u0275\u0275restoreView(_r1);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.onProcess());
    });
    \u0275\u0275conditionalCreate(16, ConverterComponent_Conditional_9_Conditional_16_Template, 2, 0);
    \u0275\u0275pipe(17, "async");
    \u0275\u0275conditionalBranchCreate(18, ConverterComponent_Conditional_9_Conditional_18_Template, 1, 0);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const meta_r3 = ctx;
...
function ConverterComponent_Conditional_17_Template(rf, ctx) {
  if (rf & 1) {
...
function ConverterComponent_Conditional_19_Template(rf, ctx) {
  if (rf & 1) {
...
var ConverterComponent = class _ConverterComponent {
  ...
  state$ = this.store.select(selectConverterPdfState);
  ...
  async onFileSelected(files) {
    ...
    try {
      ...
      this.store.dispatch(ConverterActions.loadMetaFailure({ errorCode: "FILE_CORRUPTED", message: "Could not read PDF structure." }));
    }
  }
  onProcess() {
    ...
    this.state$.subscribe((state) => {
      if (!state.inputFile)
        return;
      this.bridge.process(() => new Worker(new URL("worker-6E3IKSZI.js", import.meta.url), { type: "module" }), { file: state.inputFile }).subscribe({
        next: (msg) => {
          if (msg.type === "progress")
            this.store.dispatch(ConverterActions.updateProgress({ progress: msg.value ?? 0 }));
          if (msg.type === "complete" && msg.data) {
            this.store.dispatch(ConverterActions.processingSuccess({ outputBlob: msg.data, outputSizeMB: msg.data.size / 1048576 }));
          }
          if (msg.type === "error") {
            this.store.dispatch(ConverterActions.processingFailure({ errorCode: msg.errorCode ?? "UNKNOWN_ERROR", message: msg.message ?? "Failed", retryable: true }));
          }
        }
      });
    }).unsubscribe();
    ...
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        <h1 class="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-rose-400 drop-shadow-lg">
          \u{1F504} PDF Converter
        </h1>
        <p class="text-white/50 text-sm">Convert PDF to images or other formats.</p>
      </header>
         ...
         <div class="space-y-4">
            ...
            @if ((state$ | async)?.status === 'done') {
                <app-pdf-export-panel 
                  ...
                  defaultFilename="omni_converter.pdf" 
                />
            }
         </div>
      </div>
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ConverterComponent, { className: "ConverterComponent", filePath: "src/app/modules/pdf/04-converter/converter.component.ts", lineNumber: 82 });
})();

// src/app/modules/pdf/04-converter/converter.service.ts
var ConverterService = class _ConverterService {
  prepareConfig() {
    return {};
  }
  static \u0275fac = function ConverterService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _ConverterService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ConverterService, factory: _ConverterService.\u0275fac, providedIn: "root" });
};
(() => {
...
// src/app/modules/pdf/04-converter/converter.schema.ts
var ConverterSchema = external_exports.object({
...
export {
  ...
  ConverterService,
  converterFeature,
  selectConverterCanProcess,
  selectConverterIsLoading,
  selectConverterPdfState,
  selectOutputBlob,
  selectProgress,
  selectStatus
};
//# sourceMappingURL=chunk-ENHNOXQF.mjs.map
