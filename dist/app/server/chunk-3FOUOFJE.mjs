...
// src/app/modules/pdf/17-form-filler/formFiller.store.ts
var init = {
  inputFile: null,
  pdfMeta: null,
  status: "idle",
...
var FormFillerActions = {
  loadFile: createAction("[FormFiller] Load File", props()),
  loadMetaSuccess: createAction("[FormFiller] Meta Success", props()),
  loadMetaFailure: createAction("[FormFiller] Meta Failure", props()),
  startProcessing: createAction("[FormFiller] Start Processing"),
  updateProgress: createAction("[FormFiller] Update Progress", props()),
  processingSuccess: createAction("[FormFiller] Processing Success", props()),
  processingFailure: createAction("[FormFiller] Processing Failure", props()),
  resetState: createAction("[FormFiller] Reset State")
};
var formFillerFeature = createFeature({
  name: "formFillerPdf",
  reducer: createReducer(init, on(FormFillerActions.loadFile, (s, a) => __spreadProps(__spreadValues({}, s), { inputFile: a.file, status: "loading", outputBlob: null, errorMessage: null, progress: 0 })), on(FormFillerActions.loadMetaSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { pdfMeta: a.meta, status: "idle" })), on(FormFillerActions.loadMetaFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message })), on(FormFillerActions.startProcessing, (s) => __spreadProps(__spreadValues({}, s), { status: "processing", progress: 0 })), on(FormFillerActions.updateProgress, (s, a) => __spreadProps(__spreadValues({}, s), { progress: a.progress })), on(FormFillerActions.processingSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { status: "done", progress: 100, outputBlob: a.outputBlob, outputSizeMB: a.outputSizeMB })), on(FormFillerActions.processingFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message, retryable: a.retryable })), on(FormFillerActions.resetState, () => init))
});
var { selectFormFillerPdfState, selectStatus, selectProgress, selectOutputBlob } = formFillerFeature;
var selectFormFillerCanProcess = createSelector(selectFormFillerPdfState, (s) => !!s.inputFile && s.status === "idle");
var selectFormFillerIsLoading = createSelector(selectStatus, (s) => s === "loading" || s === "processing" || s === "rendering");

// src/app/modules/pdf/17-form-filler/formFiller.component.ts
function FormFillerComponent_Conditional_9_Conditional_16_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "div", 18);
    \u0275\u0275text(1, " Processing... ");
  }
}
function FormFillerComponent_Conditional_9_Conditional_18_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275text(0, " \u{1F4C4} Execute ");
  }
}
function FormFillerComponent_Conditional_9_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275pipe(14, "async");
    \u0275\u0275pipe(15, "async");
    \u0275\u0275listener("click", function FormFillerComponent_Conditional_9_Template_button_click_13_listener() {
      \u0275\u0275restoreView(_r1);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.onProcess());
    });
    \u0275\u0275conditionalCreate(16, FormFillerComponent_Conditional_9_Conditional_16_Template, 2, 0);
    \u0275\u0275pipe(17, "async");
    \u0275\u0275conditionalBranchCreate(18, FormFillerComponent_Conditional_9_Conditional_18_Template, 1, 0);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const meta_r3 = ctx;
...
function FormFillerComponent_Conditional_11_Template(rf, ctx) {
  if (rf & 1) {
...
function FormFillerComponent_Conditional_14_Template(rf, ctx) {
  if (rf & 1) {
...
function FormFillerComponent_Conditional_17_Template(rf, ctx) {
  if (rf & 1) {
...
function FormFillerComponent_Conditional_19_Template(rf, ctx) {
  if (rf & 1) {
...
var FormFillerComponent = class _FormFillerComponent {
  store = inject(Store);
  ...
  onProcess() {
    ...
    this.state$.subscribe((state) => {
      if (!state.inputFile)
        return;
      this.bridge.process(() => new Worker(new URL("worker-NGPJC4KB.js", import.meta.url), { type: "module" }), { file: state.inputFile }).subscribe({
        next: (msg) => {
          if (msg.type === "progress")
            this.store.dispatch(FormFillerActions.updateProgress({ progress: msg.value ?? 0 }));
  ...
  static \u0275fac = function FormFillerComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _FormFillerComponent)();
  };
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _FormFillerComponent, selectors: [["app-pdf-formFiller"]], decls: 21, vars: 17, consts: [[1, "min-h-screen", "bg-[#0a0a0f]", "p-6", "space-y-6"], [1, "space-y-1"], [1, "text-3xl", "font-bold", "text-transparent", "bg-clip-text", "bg-gradient-to-r", "from-red-400", "to-rose-400", "drop-shadow-lg"], [1, "text-white/50", "text-sm"], [1, "grid", "grid-cols-1", "lg:grid-cols-2", "gap-6"], [1, "space-y-4"], ["label", "Drop PDF file here or click", 3, "filesSelected"], [1, "p-4", "rounded-2xl", "bg-white/5", "border", "border-white/10", "space-y-4"], [1, "p-3", "rounded-xl", "bg-red-500/10", "border", "border-red-500/30", "text-sm", "text-red-400"], [3, "file"], [1, "flex", "justify-center", "p-8"], ["defaultFilename", "omni_formFiller.pdf", 3, "outputBlob", "outputSizeMB"], [1, "grid", "grid-cols-2", "gap-3", "text-center"], [1, "p-2", "rounded-lg", "bg-white/5"], [1, "text-xs", "text-white/40"], [1, "text-sm", "font-semibold", "text-rose-400"], [1, "text-sm", "font-semibold", "text-white"], [1, "w-full", "py-3", "rounded-xl", "font-semibold", "text-sm", "transition-all", "duration-300", "flex", "items-center", "justify-center", "gap-2", "bg-gradient-to-r", "from-red-500", "to-rose-600", "text-white", "hover:shadow-[0_0_30px_rgba(244,63,94,0.4)]", "disabled:opacity-40", "disabled:cursor-not-allowed", 3, "click", "disabled"], [1, "w-4", "h-4", "border-2", "border-current", "border-t-transparent", "rounded-full", "animate-spin"], ["label", "Modifying...", 3, "progress", "size"]], template: function FormFillerComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "div", 0)(1, "header", 1)(2, "h1", 2);
      \u0275\u0275text(3, " \u{1F4C4} Form Filler ");
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(4, "p", 3);
      \u0275\u0275text(5, "Fill interactive PDF forms.");
      \u0275\u0275elementEnd()();
      \u0275\u0275elementStart(6, "div", 4)(7, "div", 5)(8, "app-pdf-drop-zone", 6);
      \u0275\u0275listener("filesSelected", function FormFillerComponent_Template_app_pdf_drop_zone_filesSelected_8_listener($event) {
        return ctx.onFileSelected($event);
      });
      \u0275\u0275elementEnd();
      \u0275\u0275conditionalCreate(9, FormFillerComponent_Conditional_9_Template, 19, 13, "div", 7);
      \u0275\u0275pipe(10, "async");
      \u0275\u0275conditionalCreate(11, FormFillerComponent_Conditional_11_Template, 3, 3, "div", 8);
      \u0275\u0275pipe(12, "async");
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(13, "div", 5);
      \u0275\u0275conditionalCreate(14, FormFillerComponent_Conditional_14_Template, 2, 3, "app-pdf-preview", 9);
      \u0275\u0275pipe(15, "async");
      \u0275\u0275pipe(16, "async");
      \u0275\u0275conditionalCreate(17, FormFillerComponent_Conditional_17_Template, 3, 4, "div", 10);
      \u0275\u0275pipe(18, "async");
      \u0275\u0275conditionalCreate(19, FormFillerComponent_Conditional_19_Template, 3, 6, "app-pdf-export-panel", 11);
      \u0275\u0275pipe(20, "async");
      \u0275\u0275elementEnd()()();
    }
    if (rf & 2) {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(FormFillerComponent, [{
    type: Component,
    args: [{
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(FormFillerComponent, { className: "FormFillerComponent", filePath: "src/app/modules/pdf/17-form-filler/formfiller.component.ts", lineNumber: 82 });
})();

// src/app/modules/pdf/17-form-filler/formFiller.service.ts
var FormFillerService = class _FormFillerService {
  prepareConfig() {
    return {};
  }
  static \u0275fac = function FormFillerService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _FormFillerService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _FormFillerService, factory: _FormFillerService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(FormFillerService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();

// src/app/modules/pdf/17-form-filler/formFiller.schema.ts
var FormFillerSchema = external_exports.object({
...
export {
  ...
  FormFillerService,
  formFillerFeature,
  selectFormFillerCanProcess,
  selectFormFillerIsLoading,
  selectFormFillerPdfState,
  selectOutputBlob,
  selectProgress,
  selectStatus
};
//# sourceMappingURL=chunk-3FOUOFJE.mjs.map
