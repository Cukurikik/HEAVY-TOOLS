...
// src/app/modules/pdf/02-splitter/splitter.store.ts
var init = {
  inputFile: null,
  pdfMeta: null,
  status: "idle",
...
var SplitterActions = {
  ...
  loadMetaSuccess: createAction("[Splitter] Meta Success", props()),
  loadMetaFailure: createAction("[Splitter] Meta Failure", props()),
  startProcessing: createAction("[Splitter] Start Processing"),
  updateProgress: createAction("[Splitter] Update Progress", props()),
  processingSuccess: createAction("[Splitter] Processing Success", props()),
  processingFailure: createAction("[Splitter] Processing Failure", props()),
  resetState: createAction("[Splitter] Reset State")
};
var splitterFeature = createFeature({
  name: "splitterPdf",
  reducer: createReducer(init, on(SplitterActions.loadFile, (s, a) => __spreadProps(__spreadValues({}, s), { inputFile: a.file, status: "loading", outputBlob: null, errorMessage: null, progress: 0 })), on(SplitterActions.loadMetaSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { pdfMeta: a.meta, status: "idle" })), on(SplitterActions.loadMetaFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message })), on(SplitterActions.startProcessing, (s) => __spreadProps(__spreadValues({}, s), { status: "processing", progress: 0 })), on(SplitterActions.updateProgress, (s, a) => __spreadProps(__spreadValues({}, s), { progress: a.progress })), on(SplitterActions.processingSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { status: "done", progress: 100, outputBlob: a.outputBlob, outputSizeMB: a.outputSizeMB })), on(SplitterActions.processingFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message, retryable: a.retryable })), on(SplitterActions.resetState, () => init))
});
var { selectSplitterPdfState, selectStatus, selectProgress, selectOutputBlob } = splitterFeature;
var selectSplitterCanProcess = createSelector(selectSplitterPdfState, (s) => !!s.inputFile && s.status === "idle");
var selectSplitterIsLoading = createSelector(selectStatus, (s) => s === "loading" || s === "processing" || s === "rendering");

// src/app/modules/pdf/02-splitter/splitter.component.ts
function SplitterComponent_Conditional_9_Conditional_16_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "div", 18);
    \u0275\u0275text(1, " Processing... ");
  }
}
function SplitterComponent_Conditional_9_Conditional_18_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275text(0, " \u2702\uFE0F Execute ");
  }
}
function SplitterComponent_Conditional_9_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275pipe(14, "async");
    \u0275\u0275pipe(15, "async");
    \u0275\u0275listener("click", function SplitterComponent_Conditional_9_Template_button_click_13_listener() {
      \u0275\u0275restoreView(_r1);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.onProcess());
    });
    \u0275\u0275conditionalCreate(16, SplitterComponent_Conditional_9_Conditional_16_Template, 2, 0);
    \u0275\u0275pipe(17, "async");
    \u0275\u0275conditionalBranchCreate(18, SplitterComponent_Conditional_9_Conditional_18_Template, 1, 0);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const meta_r3 = ctx;
...
function SplitterComponent_Conditional_17_Template(rf, ctx) {
  if (rf & 1) {
...
function SplitterComponent_Conditional_19_Template(rf, ctx) {
  if (rf & 1) {
...
var SplitterComponent = class _SplitterComponent {
  ...
  state$ = this.store.select(selectSplitterPdfState);
  ...
  async onFileSelected(files) {
    ...
    try {
      ...
      this.store.dispatch(SplitterActions.loadMetaFailure({ errorCode: "FILE_CORRUPTED", message: "Could not read PDF structure." }));
    }
  }
  onProcess() {
    ...
    this.state$.subscribe((state) => {
      if (!state.inputFile)
        return;
      this.bridge.process(() => new Worker(new URL("worker-VG5NQZ5B.js", import.meta.url), { type: "module" }), { file: state.inputFile }).subscribe({
        next: (msg) => {
          if (msg.type === "progress")
            this.store.dispatch(SplitterActions.updateProgress({ progress: msg.value ?? 0 }));
          if (msg.type === "complete" && msg.data) {
            this.store.dispatch(SplitterActions.processingSuccess({ outputBlob: msg.data, outputSizeMB: msg.data.size / 1048576 }));
          }
          if (msg.type === "error") {
            this.store.dispatch(SplitterActions.processingFailure({ errorCode: msg.errorCode ?? "UNKNOWN_ERROR", message: msg.message ?? "Failed", retryable: true }));
          }
        }
      });
    }).unsubscribe();
  ...
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(SplitterComponent, [{
    ...
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Split a PDF into multiple files.</p>
      </header>
         ...
         <div class="space-y-4">
            ...
            @if ((state$ | async)?.status === 'done') {
                <app-pdf-export-panel 
                  ...
                  defaultFilename="omni_splitter.pdf" 
                />
            }
         </div>
      </div>
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(SplitterComponent, { className: "SplitterComponent", filePath: "src/app/modules/pdf/02-splitter/splitter.component.ts", lineNumber: 82 });
})();

// src/app/modules/pdf/02-splitter/splitter.service.ts
var SplitterService = class _SplitterService {
  prepareConfig() {
    return {};
  }
  static \u0275fac = function SplitterService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _SplitterService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _SplitterService, factory: _SplitterService.\u0275fac, providedIn: "root" });
};
(() => {
...
// src/app/modules/pdf/02-splitter/splitter.schema.ts
var SplitterSchema = external_exports.object({
...
export {
  ...
  selectOutputBlob,
  selectProgress,
  selectSplitterCanProcess,
  selectSplitterIsLoading,
  selectSplitterPdfState,
  selectStatus,
  splitterFeature
};
//# sourceMappingURL=chunk-E5DMCTDO.mjs.map
