...
// src/app/modules/pdf/01-merger/merger.store.ts
var init = {
  inputFile: null,
  pdfMeta: null,
  status: "idle",
...
var MergerActions = {
  ...
  loadMetaSuccess: createAction("[Merger] Meta Success", props()),
  loadMetaFailure: createAction("[Merger] Meta Failure", props()),
  startProcessing: createAction("[Merger] Start Processing"),
  updateProgress: createAction("[Merger] Update Progress", props()),
  processingSuccess: createAction("[Merger] Processing Success", props()),
  processingFailure: createAction("[Merger] Processing Failure", props()),
  resetState: createAction("[Merger] Reset State")
};
var mergerFeature = createFeature({
  name: "mergerPdf",
  reducer: createReducer(init, on(MergerActions.loadFile, (s, a) => __spreadProps(__spreadValues({}, s), { inputFile: a.file, status: "loading", outputBlob: null, errorMessage: null, progress: 0 })), on(MergerActions.loadMetaSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { pdfMeta: a.meta, status: "idle" })), on(MergerActions.loadMetaFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message })), on(MergerActions.startProcessing, (s) => __spreadProps(__spreadValues({}, s), { status: "processing", progress: 0 })), on(MergerActions.updateProgress, (s, a) => __spreadProps(__spreadValues({}, s), { progress: a.progress })), on(MergerActions.processingSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { status: "done", progress: 100, outputBlob: a.outputBlob, outputSizeMB: a.outputSizeMB })), on(MergerActions.processingFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message, retryable: a.retryable })), on(MergerActions.resetState, () => init))
});
var { selectMergerPdfState, selectStatus, selectProgress, selectOutputBlob } = mergerFeature;
var selectMergerCanProcess = createSelector(selectMergerPdfState, (s) => !!s.inputFile && s.status === "idle");
var selectMergerIsLoading = createSelector(selectStatus, (s) => s === "loading" || s === "processing" || s === "rendering");

// src/app/modules/pdf/01-merger/merger.component.ts
function MergerComponent_Conditional_9_Conditional_16_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "div", 18);
    \u0275\u0275text(1, " Processing... ");
  }
}
function MergerComponent_Conditional_9_Conditional_18_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275text(0, " \u{1F517} Execute ");
  }
}
function MergerComponent_Conditional_9_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275conditionalCreate(16, MergerComponent_Conditional_9_Conditional_16_Template, 2, 0);
    \u0275\u0275pipe(17, "async");
    \u0275\u0275conditionalBranchCreate(18, MergerComponent_Conditional_9_Conditional_18_Template, 1, 0);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const meta_r3 = ctx;
...
function MergerComponent_Conditional_11_Template(rf, ctx) {
  if (rf & 1) {
...
function MergerComponent_Conditional_14_Template(rf, ctx) {
  if (rf & 1) {
...
function MergerComponent_Conditional_17_Template(rf, ctx) {
  if (rf & 1) {
...
function MergerComponent_Conditional_19_Template(rf, ctx) {
  if (rf & 1) {
...
var MergerComponent = class _MergerComponent {
  ...
  onProcess() {
    ...
    this.state$.subscribe((state) => {
      if (!state.inputFile)
        return;
      this.bridge.process(() => new Worker(new URL("worker-63ML5SBG.js", import.meta.url), { type: "module" }), { file: state.inputFile }).subscribe({
        next: (msg) => {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(MergerComponent, { className: "MergerComponent", filePath: "src/app/modules/pdf/01-merger/merger.component.ts", lineNumber: 82 });
})();

// src/app/modules/pdf/01-merger/merger.service.ts
...
// src/app/modules/pdf/01-merger/merger.schema.ts
var MergerSchema = external_exports.object({
...
export {
  ...
  MergerService,
  mergerFeature,
  selectMergerCanProcess,
  selectMergerIsLoading,
  selectMergerPdfState,
  selectOutputBlob,
  selectProgress,
  selectStatus
};
//# sourceMappingURL=chunk-33ZFSFFW.mjs.map
