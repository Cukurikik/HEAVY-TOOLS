...
// src/app/modules/pdf/06-watermark/watermark.store.ts
var init = {
  inputFile: null,
  pdfMeta: null,
  status: "idle",
...
var WatermarkActions = {
  ...
  loadMetaSuccess: createAction("[Watermark] Meta Success", props()),
  loadMetaFailure: createAction("[Watermark] Meta Failure", props()),
  startProcessing: createAction("[Watermark] Start Processing"),
  updateProgress: createAction("[Watermark] Update Progress", props()),
  processingSuccess: createAction("[Watermark] Processing Success", props()),
  processingFailure: createAction("[Watermark] Processing Failure", props()),
  resetState: createAction("[Watermark] Reset State")
};
var watermarkFeature = createFeature({
  name: "watermarkPdf",
  reducer: createReducer(init, on(WatermarkActions.loadFile, (s, a) => __spreadProps(__spreadValues({}, s), { inputFile: a.file, status: "loading", outputBlob: null, errorMessage: null, progress: 0 })), on(WatermarkActions.loadMetaSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { pdfMeta: a.meta, status: "idle" })), on(WatermarkActions.loadMetaFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message })), on(WatermarkActions.startProcessing, (s) => __spreadProps(__spreadValues({}, s), { status: "processing", progress: 0 })), on(WatermarkActions.updateProgress, (s, a) => __spreadProps(__spreadValues({}, s), { progress: a.progress })), on(WatermarkActions.processingSuccess, (s, a) => __spreadProps(__spreadValues({}, s), { status: "done", progress: 100, outputBlob: a.outputBlob, outputSizeMB: a.outputSizeMB })), on(WatermarkActions.processingFailure, (s, a) => __spreadProps(__spreadValues({}, s), { status: "error", errorCode: a.errorCode, errorMessage: a.message, retryable: a.retryable })), on(WatermarkActions.resetState, () => init))
});
var { selectWatermarkPdfState, selectStatus, selectProgress, selectOutputBlob } = watermarkFeature;
var selectWatermarkCanProcess = createSelector(selectWatermarkPdfState, (s) => !!s.inputFile && s.status === "idle");
var selectWatermarkIsLoading = createSelector(selectStatus, (s) => s === "loading" || s === "processing" || s === "rendering");

// src/app/modules/pdf/06-watermark/watermark.component.ts
function WatermarkComponent_Conditional_9_Conditional_16_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "div", 18);
    \u0275\u0275text(1, " Processing... ");
  }
}
function WatermarkComponent_Conditional_9_Conditional_18_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275text(0, " \u{1F4A7} Execute ");
  }
}
function WatermarkComponent_Conditional_9_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275pipe(14, "async");
    \u0275\u0275pipe(15, "async");
    \u0275\u0275listener("click", function WatermarkComponent_Conditional_9_Template_button_click_13_listener() {
      \u0275\u0275restoreView(_r1);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.onProcess());
    });
    \u0275\u0275conditionalCreate(16, WatermarkComponent_Conditional_9_Conditional_16_Template, 2, 0);
    \u0275\u0275pipe(17, "async");
    \u0275\u0275conditionalBranchCreate(18, WatermarkComponent_Conditional_9_Conditional_18_Template, 1, 0);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const meta_r3 = ctx;
...
function WatermarkComponent_Conditional_17_Template(rf, ctx) {
  if (rf & 1) {
...
function WatermarkComponent_Conditional_19_Template(rf, ctx) {
  if (rf & 1) {
...
var WatermarkComponent = class _WatermarkComponent {
  ...
  state$ = this.store.select(selectWatermarkPdfState);
  ...
  async onFileSelected(files) {
    ...
    try {
      ...
      this.store.dispatch(WatermarkActions.loadMetaFailure({ errorCode: "FILE_CORRUPTED", message: "Could not read PDF structure." }));
    }
  }
  onProcess() {
    ...
    this.state$.subscribe((state) => {
      if (!state.inputFile)
        return;
      this.bridge.process(() => new Worker(new URL("worker-YE7E75SC.js", import.meta.url), { type: "module" }), { file: state.inputFile }).subscribe({
        next: (msg) => {
          if (msg.type === "progress")
            this.store.dispatch(WatermarkActions.updateProgress({ progress: msg.value ?? 0 }));
          if (msg.type === "complete" && msg.data) {
            this.store.dispatch(WatermarkActions.processingSuccess({ outputBlob: msg.data, outputSizeMB: msg.data.size / 1048576 }));
          }
          if (msg.type === "error") {
            this.store.dispatch(WatermarkActions.processingFailure({ errorCode: msg.errorCode ?? "UNKNOWN_ERROR", message: msg.message ?? "Failed", retryable: true }));
          }
        }
      });
    }).unsubscribe();
    ...
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        <h1 class="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-rose-400 drop-shadow-lg">
          \u{1F4A7} PDF Watermark
        </h1>
        <p class="text-white/50 text-sm">Add image/text watermark.</p>
      </header>
         ...
         <div class="space-y-4">
            ...
            @if ((state$ | async)?.status === 'done') {
                <app-pdf-export-panel 
                  ...
                  defaultFilename="omni_watermark.pdf" 
                />
            }
         </div>
      </div>
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(WatermarkComponent, { className: "WatermarkComponent", filePath: "src/app/modules/pdf/06-watermark/watermark.component.ts", lineNumber: 82 });
})();

// src/app/modules/pdf/06-watermark/watermark.service.ts
var WatermarkService = class _WatermarkService {
  prepareConfig() {
    return {};
  }
  static \u0275fac = function WatermarkService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _WatermarkService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _WatermarkService, factory: _WatermarkService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(WatermarkService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();

// src/app/modules/pdf/06-watermark/watermark.schema.ts
...
export {
  ...
  selectProgress,
  selectStatus,
  selectWatermarkCanProcess,
  selectWatermarkIsLoading,
  selectWatermarkPdfState,
  watermarkFeature
};
//# sourceMappingURL=chunk-LUVWDRUL.mjs.map
