  ...
  try {
    ...
    for (; ; ) {
      ...
      if (done2) {
        ...
        cb && cb({ url, total, received, delta, done: done2 });
        break;
      }
      chunks.push(value);
      received += delta;
...
var toBlobURL = async (url, mimeType, progress2 = false, cb) => {
  const buf = progress2 ? await downloadWithProgress(url, cb) : await (await fetch(url)).arrayBuffer();
  const blob = new Blob([buf], { type: mimeType });
  return URL.createObjectURL(blob);
};

// src/app/modules/video/03-converter/converter.worker.ts
var ffmpeg = null;
...
function progress(v) {
  postMessage({ type: "progress", value: v });
}
function done(blob) {
  postMessage({ type: "complete", data: blob });
}
function fail(msg) {
  postMessage({ type: "error", errorCode: "FFMPEG_COMMAND_FAILED", message: msg });
}
addEventListener("message", async (e) => {
  ...
  try {
    ...
    let args = [];
    const presetMap = { fast: "ultrafast", balanced: "medium", best: "slow" };
    ...
    const mimeMap = { mp4: "video/mp4", webm: "video/webm", mov: "video/quicktime", avi: "video/x-msvideo", mkv: "video/x-matroska", gif: "image/gif" };
    done(new Blob([new Uint8Array(data)], { type: mimeMap[targetFormat] || "video/mp4" }));
...
//# sourceMappingURL=worker-HXKY6WDV.js.map
