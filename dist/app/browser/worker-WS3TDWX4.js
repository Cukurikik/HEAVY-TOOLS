...
// src/app/modules/video/06-reverser/reverser.worker.ts
var ffmpeg = null;
...
addEventListener("message", async (e) => {
  ...
  try {
    ...
    const { file, reverseAudio = true } = config;
    const inName = "in.mp4";
    const outName = "reversed.mp4";
    ffmpeg.writeFile(inName, await fetchFile(file));
    const vf = "reverse";
    let args = reverseAudio ? ["-i", inName, "-vf", vf, "-af", "areverse", "-c:v", "libx264", "-preset", "ultrafast", "-c:a", "aac", outName] : ["-i", inName, "-vf", vf, "-an", "-c:v", "libx264", "-preset", "ultrafast", outName];
    await ffmpeg.exec(args);
    const data = await ffmpeg.readFile(outName);
    done(new Blob([new Uint8Array(data)], { type: "video/mp4" }));
...
//# sourceMappingURL=worker-WS3TDWX4.js.map
