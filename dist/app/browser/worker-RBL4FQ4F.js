...
// src/app/modules/video/11-color-grading/colorGrading.worker.ts
var ffmpeg = null;
...
addEventListener("message", async (e) => {
  ...
  try {
    ...
    const { file, brightness = 0, contrast = 1, saturation = 1, gamma = 1 } = config;
    const inName = "in.mp4";
    const outName = "color.mp4";
    ffmpeg.writeFile(inName, await fetchFile(file));
    const vf = `eq=brightness=${brightness}:contrast=${contrast}:saturation=${saturation}:gamma=${gamma}`;
...
//# sourceMappingURL=worker-RBL4FQ4F.js.map
