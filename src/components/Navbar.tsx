    ...
    >
      <div className="flex items-center justify-between px-8 py-5 max-w-[1800px] mx-auto">
        ...
        <div className="flex items-center gap-8">
          <button className="hidden md:flex items-center gap-3 px-4 py-2 rounded-full bg-white/5 border border-white/5 text-slate-400 hover:text-white hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 transition-all cursor-pointer group">
...
function NavLink({ href, label }: { href: string, label: string }) {
  return (
    <Link href={href} className="text-xs font-black text-slate-400 hover:text-white tracking-widest uppercase transition-colors relative group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 rounded-sm">
      {label}
...
function IconButton({ icon, ariaLabel }: { icon: React.ReactNode, ariaLabel: string }) {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.9 }}
      className="p-2.5 rounded-xl bg-white/5 border border-white/5 text-slate-400 hover:text-white hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 transition-all"
    >
      {icon}
    </motion.button>
  );
