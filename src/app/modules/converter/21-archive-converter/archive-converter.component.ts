  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Convert compressed archives between ZIP, 7Z, TAR, and GZIP</p>
      </header>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="space-y-4">
          <app-file-drop-zone
            ...
            [multiple]="false"
            label="Drop archive file here or click to browse"
        ...
        <div class="space-y-4">
          ...
          @if ((state$ | async)?.status === 'done') {
            <app-converter-export-panel
              ...
              [filename]="'converted_archive.' + (state$ | async)?.outputFormat"
              (download)="onDownload()" />
          }
        </div>
      </div>
  ...
  onProcess(): void {
    this.state$.pipe(take(1)).subscribe(state => {
      ...
      this.bridge.process<any, { blob: Blob; text: string }>(
        () => new Worker(new URL('./archive-converter.worker', import.meta.url), { type: 'module' }),
        { file: state.inputFile, outputFormat: state.outputFormat, compressionLevel: state.compressionLevel }
      ).subscribe({
        next: (msg) => {
          if (msg.type === 'progress') {
            this.store.dispatch(ArchiveConverterActions.updateProgress({ progress: msg.value ?? 0 }));
          } else if (msg.type === 'complete' && msg.data) {
            const blob = msg.data.blob;
            this.store.dispatch(ArchiveConverterActions.processingSuccess({ outputBlob: blob, outputSizeMB: blob.size / 1048576 }));
          } else if (msg.type === 'error') {
            this.store.dispatch(ArchiveConverterActions.processingFailure({ errorCode: msg.errorCode ?? 'UNKNOWN_ERROR', message: msg.message ?? 'Archive conversion failed', retryable: true }));
          }
        },
        error: (err) => {
          this.store.dispatch(ArchiveConverterActions.processingFailure({ errorCode: 'WORKER_CRASHED', message: String(err), retryable: true }));
        }
      });
    });
  }
  onDownload(): void {
    this.store.dispatch(ArchiveConverterActions.downloadOutput());
  }
  ngOnDestroy(): void {
