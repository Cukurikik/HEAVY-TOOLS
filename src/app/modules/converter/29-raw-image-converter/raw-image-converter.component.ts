...
const OUTPUT_FORMATS: FormatOption[] = [
  { value: 'jpg', label: 'JPEG (High)', icon: '🖼️' },
  { value: 'png', label: 'PNG (Lossless)', icon: '🖼️' },
  { value: 'tiff', label: 'TIFF (16-bit)', icon: '📄' },
  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Develop RAW photography files (DNG, CR2, NEF, ARW) into web-ready formats</p>
      </header>

      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="space-y-4">
          <app-file-drop-zone
            accept=".dng,.cr2,.nef,.arw,.orf,.raf"
            [multiple]="false"
            label="Drop RAW file here or click to browse"
            (filesSelected)="onFilesSelected($event)" />

          <app-converter-format-selector
            [formats]="outputFormats"
            [selected]="((state$ | async)?.outputFormat ?? 'jpg')"
        ...
        <div class="space-y-4">
          ...
          @if ((state$ | async)?.status === 'done') {
            <app-converter-export-panel
              ...
              [filename]="'developed_image.' + (state$ | async)?.outputFormat"
              (download)="onDownload()" />
          }
        </div>
      </div>
  ...
  onProcess(): void {
    this.state$.pipe(take(1)).subscribe(state => {
      ...
      this.bridge.process<any, { blob: Blob; text: string }>(
        () => new Worker(new URL('./raw-image-converter.worker', import.meta.url), { type: 'module' }),
        { file: state.inputFile, outputFormat: state.outputFormat, exposure: state.exposure, contrast: state.contrast }
      ).subscribe({
        next: (msg) => {
          if (msg.type === 'progress') {
            this.store.dispatch(RawImageConverterActions.updateProgress({ progress: msg.value ?? 0 }));
          } else if (msg.type === 'complete' && msg.data) {
            const blob = msg.data.blob;
            this.store.dispatch(RawImageConverterActions.processingSuccess({ outputBlob: blob, outputSizeMB: blob.size / 1048576 }));
          } else if (msg.type === 'error') {
            this.store.dispatch(RawImageConverterActions.processingFailure({ errorCode: msg.errorCode ?? 'UNKNOWN_ERROR', message: msg.message ?? 'RAW Development failed', retryable: true }));
          }
        },
        error: (err) => {
          this.store.dispatch(RawImageConverterActions.processingFailure({ errorCode: 'WORKER_CRASHED', message: String(err), retryable: true }));
        }
      });
    });
  }
  onDownload(): void {
    this.store.dispatch(RawImageConverterActions.downloadOutput());
  }
  ngOnDestroy(): void {
