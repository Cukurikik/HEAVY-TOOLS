  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Convert numbers between Binary, Octal, Decimal, Hex, and Base64</p>
      </header>
        ...
        <div class="space-y-4">
          <div class="relative group">
            <textarea
              ...
              placeholder="Enter number (e.g. 255 or 0xFF)..."
...
export class NumberBaseConverterComponent implements OnDestroy {
  ...
  onTextInput(text: string): void {
    this.store.dispatch(NumberBaseConverterActions.setInputText({ text }));
  }
  ...
  onProcess(): void {
    this.state$.pipe(take(1)).subscribe(state => {
      ...
      this.bridge.process<any, { blob: Blob; text: string }>(
        () => new Worker(new URL('./number-base-converter.worker', import.meta.url), { type: 'module' }),
        { inputText: state.inputText, outputFormat: state.outputFormat, fromBase: state.fromBase }
      ).subscribe({
        next: (msg) => {
          if (msg.type === 'progress') {
            this.store.dispatch(NumberBaseConverterActions.updateProgress({ progress: msg.value ?? 0 }));
          } else if (msg.type === 'complete' && msg.data) {
            const blob = msg.data.blob;
            this.store.dispatch(NumberBaseConverterActions.processingSuccess({ outputBlob: blob, outputSizeMB: blob.size / 1048576 }));
          } else if (msg.type === 'error') {
            this.store.dispatch(NumberBaseConverterActions.processingFailure({ errorCode: msg.errorCode ?? 'UNKNOWN_ERROR', message: msg.message ?? 'Conversion failed', retryable: true }));
          }
        },
        error: (err) => {
          this.store.dispatch(NumberBaseConverterActions.processingFailure({ errorCode: 'WORKER_CRASHED', message: String(err), retryable: true }));
        }
      });
    });
  }
  onDownload(): void {
    this.store.dispatch(NumberBaseConverterActions.downloadOutput());
  }
  ngOnDestroy(): void {
