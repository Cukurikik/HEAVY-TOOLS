// ============================================================
// FEATURE 05 — IMAGE RESIZER — Component
        ...
        <div class="space-y-4">
          ...
          @if ((state$ | async)?.status === 'done') {
            <app-converter-export-panel
              ...
              [filename]="'resized_' + (state$ | async)?.inputFile?.name"
              (download)="onDownload()" />
          }
        </div>
      </div>
  ...
  onProcess(): void {
    this.state$.pipe(take(1)).subscribe(state => {
      ...
      this.bridge.process<any, { blob: Blob; text: string }>(
        () => new Worker(new URL('./image-resizer.worker', import.meta.url), { type: 'module' }),
        { file: state.inputFile, outputFormat: state.outputFormat, width: state.width, height: state.height, maintainAspectRatio: state.maintainAspectRatio }
      ).subscribe({
        next: (msg) => {
          if (msg.type === 'progress') {
            this.store.dispatch(ImageResizerActions.updateProgress({ progress: msg.value ?? 0 }));
          } else if (msg.type === 'complete' && msg.data) {
            const blob = msg.data.blob;
            this.store.dispatch(ImageResizerActions.processingSuccess({ outputBlob: blob, outputSizeMB: blob.size / 1048576 }));
          } else if (msg.type === 'error') {
            this.store.dispatch(ImageResizerActions.processingFailure({ errorCode: msg.errorCode ?? 'UNKNOWN_ERROR', message: msg.message ?? 'Resizing failed', retryable: true }));
          }
        },
        error: (err) => {
          this.store.dispatch(ImageResizerActions.processingFailure({ errorCode: 'WORKER_CRASHED', message: String(err), retryable: true }));
        }
      });
    });
  }
  onDownload(): void {
    this.store.dispatch(ImageResizerActions.downloadOutput());
  }
  ngOnDestroy(): void {
