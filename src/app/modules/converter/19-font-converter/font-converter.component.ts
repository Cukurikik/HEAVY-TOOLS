// ============================================================
// FEATURE 19 — FONT CONVERTER — Component
...
import { FontConverterActions, selectFontConverterState } from './font-converter.store';
import { ConverterWorkerBridgeService } from '../shared/engine/worker-bridge.service';
import { take } from 'rxjs';

const OUTPUT_FORMATS: FormatOption[] = [
  { value: 'ttf', label: 'TTF', icon: 'font' },
  { value: 'otf', label: 'OTF', icon: 'font' },
  { value: 'woff', label: 'WOFF', icon: '🌐' },
  { value: 'woff2', label: 'WOFF2', icon: '🌐' },
...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Convert fonts between TTF, OTF, WOFF, WOFF2, and SVG formats</p>
      </header>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="space-y-4">
          <app-file-drop-zone
            ...
            [multiple]="false"
            label="Drop font file here or click to browse"
            (filesSelected)="onFilesSelected($event)" />

          <app-converter-format-selector
            [formats]="outputFormats"
            [selected]="((state$ | async)?.outputFormat ?? 'woff2')"
        ...
        <div class="space-y-4">
          ...
          @if ((state$ | async)?.status === 'done') {
            <app-converter-export-panel
              ...
              [filename]="'font_converted.' + (state$ | async)?.outputFormat"
              (download)="onDownload()" />
          }
        </div>
      </div>
  ...
  ` })
export class FontConverterComponent implements OnDestroy {
  ...
  onProcess(): void {
    this.state$.pipe(take(1)).subscribe(state => {
      ...
      this.bridge.process<any, { blob: Blob; text: string }>(
        () => new Worker(new URL('./font-converter.worker', import.meta.url), { type: 'module' }),
        { file: state.inputFile, outputFormat: state.outputFormat }
      ).subscribe({
        next: (msg) => {
          if (msg.type === 'progress') {
            this.store.dispatch(FontConverterActions.updateProgress({ progress: msg.value ?? 0 }));
          } else if (msg.type === 'complete' && msg.data) {
            const blob = msg.data.blob;
            this.store.dispatch(FontConverterActions.processingSuccess({ outputBlob: blob, outputSizeMB: blob.size / 1048576 }));
          } else if (msg.type === 'error') {
            this.store.dispatch(FontConverterActions.processingFailure({ errorCode: msg.errorCode ?? 'UNKNOWN_ERROR', message: msg.message ?? 'Font conversion failed', retryable: true }));
          }
        },
        error: (err) => {
          this.store.dispatch(FontConverterActions.processingFailure({ errorCode: 'WORKER_CRASHED', message: String(err), retryable: true }));
        }
      });
    });
  }
  onDownload(): void {
    this.store.dispatch(FontConverterActions.downloadOutput());
  }
  ngOnDestroy(): void {
