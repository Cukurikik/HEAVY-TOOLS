  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Convert JSON to YAML, XML, CSV, or minify/beautify</p>
      </header>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="space-y-4">
          <app-file-drop-zone
            accept=".json"
            [multiple]="false"
            label="Drop JSON file here or use the editor below"
...
export class JsonConverterComponent implements OnDestroy {
  ...
  onTextInput(text: string): void {
    this.store.dispatch(JsonConverterActions.setInputText({ text }));
  }
  ...
  onProcess(): void {
    this.state$.pipe(take(1)).subscribe(state => {
      ...
      this.bridge.process<any, { blob: Blob; text: string }>(
        () => new Worker(new URL('./json-converter.worker', import.meta.url), { type: 'module' }),
        { 
        ...
        next: (msg) => {
          if (msg.type === 'progress') {
            this.store.dispatch(JsonConverterActions.updateProgress({ progress: msg.value ?? 0 }));
          } else if (msg.type === 'complete' && msg.data) {
            const blob = msg.data.blob;
            this.store.dispatch(JsonConverterActions.processingSuccess({ outputBlob: blob, outputSizeMB: blob.size / 1048576 }));
          } else if (msg.type === 'error') {
            this.store.dispatch(JsonConverterActions.processingFailure({ errorCode: msg.errorCode ?? 'UNKNOWN_ERROR', message: msg.message ?? 'Conversion failed', retryable: true }));
          }
        },
        error: (err) => {
          this.store.dispatch(JsonConverterActions.processingFailure({ errorCode: 'WORKER_CRASHED', message: String(err), retryable: true }));
        }
      });
    });
  }
  onDownload(): void {
    this.store.dispatch(JsonConverterActions.downloadOutput());
  }
  ngOnDestroy(): void {
