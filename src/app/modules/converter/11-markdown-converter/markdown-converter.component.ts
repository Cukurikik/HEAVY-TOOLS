// ============================================================
// FEATURE 11 — MARKDOWN CONVERTER — Component
  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Convert Markdown to HTML, PDF, Word, or plain text</p>
      </header>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="space-y-4">
          <app-file-drop-zone
            accept=".md,.markdown"
            [multiple]="false"
            label="Drop Markdown file here or use the editor below"
...
export class MarkdownConverterComponent implements OnDestroy {
  ...
  onTextInput(text: string): void {
    this.store.dispatch(MarkdownConverterActions.setInputText({ text }));
  }
  ...
  onProcess(): void {
    this.state$.pipe(take(1)).subscribe(state => {
      ...
      this.bridge.process<any, { blob: Blob; text: string }>(
        () => new Worker(new URL('./markdown-converter.worker', import.meta.url), { type: 'module' }),
        { 
        ...
        next: (msg) => {
          if (msg.type === 'progress') {
            this.store.dispatch(MarkdownConverterActions.updateProgress({ progress: msg.value ?? 0 }));
          } else if (msg.type === 'complete' && msg.data) {
            const blob = msg.data.blob;
            this.store.dispatch(MarkdownConverterActions.processingSuccess({ outputBlob: blob, outputSizeMB: blob.size / 1048576 }));
          } else if (msg.type === 'error') {
            this.store.dispatch(MarkdownConverterActions.processingFailure({ errorCode: msg.errorCode ?? 'UNKNOWN_ERROR', message: msg.message ?? 'Conversion failed', retryable: true }));
          }
        },
        error: (err) => {
          this.store.dispatch(MarkdownConverterActions.processingFailure({ errorCode: 'WORKER_CRASHED', message: String(err), retryable: true }));
        }
      });
    });
  }
  onDownload(): void {
    this.store.dispatch(MarkdownConverterActions.downloadOutput());
  }
  ngOnDestroy(): void {
