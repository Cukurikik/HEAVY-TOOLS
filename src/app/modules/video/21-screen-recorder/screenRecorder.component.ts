...
import { ScreenRecorderActions, selectScreenRecorderState, selectScreenRecorderIsLoading, selectScreenRecorderCanProcess } from './screenRecorder.store';
  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Record screen and microphone/system audio using MediaRecorder API</p>
      </header>
        ...
        <div class="space-y-4">
          ...
          @if ((state$ | async)?.status === 'done') {
            <app-export-panel
              ...
              defaultFilename="omni_screen_recorder"
            />
          }
        </div>
      </div>
...
export class ScreenRecorderComponent implements OnDestroy {
  ...
  canProcess$ = this.store.select(selectScreenRecorderCanProcess);

  async onFileSelected(files: File[]) {
    const file = files[0];
    this.store.dispatch(ScreenRecorderActions.loadFile({ file }));
    try {
      const meta = await this.ffmpeg.getMetadata(file);
      this.store.dispatch(ScreenRecorderActions.loadMetaSuccess({ meta }));
    } catch {
      this.store.dispatch(ScreenRecorderActions.loadMetaFailure({
  ...
  onProcess() {
    this.store.dispatch(ScreenRecorderActions.startProcessing());
    this.state$.subscribe(state => {
      if (!state.inputFile) return;
      this.bridge.process<unknown, Blob>(
        () => new Worker(new URL('./screenRecorder.worker', import.meta.url), { type: 'module' }),
