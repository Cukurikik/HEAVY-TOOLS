  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Upscale video resolution using Real-ESRGAN ONNX model via WebGPU acceleration</p>
      </header>
...
export class UpscalerComponent implements OnDestroy {
  ...
  async onFileSelected(files: File[]) {
    ...
    try {
      ...
      this.store.dispatch(UpscalerActions.loadMetaFailure({
