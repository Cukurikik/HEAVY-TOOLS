  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Extract frames from video as images or generate contact sheets</p>
      </header>
        ...
        <div class="space-y-4">
          ...
          @if ((state$ | async)?.status === 'done') {
            <app-export-panel
              ...
              defaultFilename="omni_thumbnail_generator"
            />
          }
        </div>
      </div>
...
export class ThumbnailGeneratorComponent implements OnDestroy {
  ...
  async onFileSelected(files: File[]) {
    ...
    try {
      ...
      this.store.dispatch(ThumbnailGeneratorActions.loadMetaFailure({
