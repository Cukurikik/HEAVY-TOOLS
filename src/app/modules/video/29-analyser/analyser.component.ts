  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Inspect video technical metadata: codec, resolution, bitrate, FPS, audio streams, chapters</p>
      </header>
...
export class AnalyserComponent implements OnDestroy {
  ...
  async onFileSelected(files: File[]) {
    ...
    try {
      ...
      this.store.dispatch(AnalyserActions.loadMetaFailure({
    ...
    this.state$.subscribe(state => {
      ...
      this.bridge.process<unknown, Blob>(
        ...
        if (msg.type === 'progress') {
          ...
          this.store.dispatch(AnalyserActions.processingSuccess({
            outputBlob: blob,
            outputSizeMB: blob.size / 1_048_576
          }));
        } else if (msg.type === 'error') {
          this.store.dispatch(AnalyserActions.processingFailure({
