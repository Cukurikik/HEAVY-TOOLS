  ...
  template: `
    ...
    >
      <div leftPanel class="space-y-4">
        <app-file-drop-zone
          accept="video/*"
          label="Drop video clips here (multiple files)"
          [multiple]="true"
        ...
        @if (clips.length > 0) {
          <div
            class="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-3"
          >
            <div class="flex justify-between items-center">
              <span class="text-xs text-white/40 uppercase tracking-wider"
                >Clips ({{ clips.length }})</span
              >
              <button
                (click)="clearClips()"
            ...
            <div class="space-y-2 max-h-64 overflow-y-auto">
              @for (clip of clips; track $index) {
                ...
                >
                  <span
                    class="w-6 h-6 flex items-center justify-center rounded-full bg-blue-500/20 text-blue-400 text-xs font-bold"
                    >{{ $index + 1 }}</span
                  >
                  <div class="flex-1 min-w-0">
                    <p class="text-sm text-white truncate">{{ clip.name }}</p>
                    <p class="text-xs text-white/30">
                      {{ clip.size / 1_048_576 | number: "1.0-1" }} MB
                    </p>
                  </div>
                  <button
                    (click)="removeClip($index)"
                    class="text-xs text-red-400 hover:text-red-300 px-2 py-1 rounded-lg hover:bg-red-500/10 transition-all"
                  >
                    ✕
                  </button>
                </div>
            ...
            <button
              ...
              class="w-full py-2 rounded-lg border border-dashed border-white/20 text-xs text-white/40 hover:text-white/60 hover:border-white/40 transition-all"
            >
              + Add More Clips
            </button>
            <input
              ...
              multiple
              class="hidden"
              (change)="onAddMore($event)"
            />

            <button
              [disabled]="clips.length < 2 || (isLoading$ | async)"
              (click)="onProcess()"
  ...
  onProcess() {
    ...
    this.bridge
      .process<unknown, Blob>(
        () =>
          new Worker(new URL("./merger.worker", import.meta.url), {
            type: "module",
          }),
      ...
      .subscribe((msg) => {
        if (msg.type === "progress")
          this.store.dispatch(
            MergerActions.updateProgress({ progress: msg.value ?? 0 }),
          );
        else if (msg.type === "complete" && msg.data) {
          const blob = msg.data as Blob;
          this.store.dispatch(
            MergerActions.processingSuccess({
              outputBlob: blob,
          ...
          this.store.dispatch(
            MergerActions.processingFailure({
