  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Crop video to custom region or resize to preset dimensions with aspect ratio lock</p>
      </header>
        ...
        <div class="space-y-4">
          ...
          @if ((state$ | async)?.status === 'done') {
            <app-export-panel
              ...
              defaultFilename="omni_crop_resize"
            />
          }
        </div>
      </div>
...
export class CropResizeComponent implements OnDestroy {
  ...
  async onFileSelected(files: File[]) {
    ...
    try {
      ...
      this.store.dispatch(CropResizeActions.loadMetaFailure({
