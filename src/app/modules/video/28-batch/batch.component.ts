  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Apply the same operation to multiple videos in a sequential processing queue</p>
      </header>
...
export class BatchComponent implements OnDestroy {
  ...
  async onFileSelected(files: File[]) {
    const file = files[0];
    this.store.dispatch(BatchActions.loadFile({ file }));
    try {
      const meta = await this.ffmpeg.getMetadata(file);
      this.store.dispatch(BatchActions.loadMetaSuccess({ meta }));
    } catch {
      this.store.dispatch(BatchActions.loadMetaFailure({
    ...
    this.state$.subscribe(state => {
      ...
      this.bridge.process<unknown, Blob>(
        ...
        if (msg.type === 'progress') {
          ...
          this.store.dispatch(BatchActions.processingSuccess({
            outputBlob: blob,
            outputSizeMB: blob.size / 1_048_576
          }));
        } else if (msg.type === 'error') {
          this.store.dispatch(BatchActions.processingFailure({
