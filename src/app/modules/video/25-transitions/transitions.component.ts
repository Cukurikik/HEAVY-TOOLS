  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Add fade, dissolve, wipe, and slide transition effects between clips</p>
      </header>
        ...
        <div class="space-y-4">
          ...
          @if ((state$ | async)?.status === 'done') {
            <app-export-panel
              ...
              defaultFilename="omni_transitions"
            />
          }
        </div>
      </div>
...
export class TransitionsComponent implements OnDestroy {
  ...
  async onFileSelected(files: File[]) {
    ...
    try {
      ...
      this.store.dispatch(TransitionsActions.loadMetaFailure({
