...
import { AudioRemoverActions, selectAudioRemoverState, selectStatus, selectProgress, selectOutputBlob } from './audio-remover.store';
import { AudioRemoverService } from './audio-remover.service';

@Component({
  selector: 'app-audio-remover',
  standalone: true,
    ...
    <div class="p-8 max-w-4xl mx-auto space-y-8">
      <header class="space-y-2">
        <h1 class="text-3xl font-bold flex items-center gap-3">
          <mat-icon class="text-accent-cyan">no_music</mat-icon>
          Audio Remover
        </h1>
        <p class="text-text-secondary">Instantly strip audio tracks from your videos. Fast and efficient.</p>
      </header>
        ...
        @if ((status$ | async) === null || (status$ | async) === undefined || (status$ | async) === 'idle') {
          ...
          <div class="flex flex-col items-center gap-6 w-full max-w-md">
            ...
            <p class="text-accent-cyan animate-pulse font-medium">
              {{ (status$ | async) === 'loading' ? 'Analyzing Video...' : 'Stripping Audio...' }}
            </p>
          </div>
          ...
          <div class="flex flex-col items-center gap-6 text-center">
            ...
            <div>
              <p class="text-xl font-bold">Audio Stripped!</p>
              <p class="text-sm text-text-secondary">Your video is now silent.</p>
            </div>
...
export class AudioRemoverComponent {
  private store = inject(Store);
  public service = inject(AudioRemoverService);

  state$ = this.store.select(selectAudioRemoverState);
  ...
  onFileSelected(event: Event) {
    ...
    if (file) {
      this.store.dispatch(AudioRemoverActions.loadFile({ file }));
      
      // Mock meta loading
      setTimeout(() => {
        this.store.dispatch(AudioRemoverActions.loadMetaSuccess({
          meta: {
            duration: 120,
            width: 1920,
            height: 1080,
  ...
  start() {
    this.store.dispatch(AudioRemoverActions.startProcessing());
    
    const worker = new Worker(new URL('./audio-remover.worker', import.meta.url));
    ...
    worker.onmessage = ({ data }) => {
      if (data.type === 'progress') {
        this.store.dispatch(AudioRemoverActions.updateProgress({ progress: data.value }));
      } else if (data.type === 'complete') {
        this.store.dispatch(AudioRemoverActions.processingSuccess({ 
          outputBlob: data.data, 
          outputSizeMB: data.data.size / 1024 / 1024 
        }));
        worker.terminate();
      } else if (data.type === 'error') {
        this.store.dispatch(AudioRemoverActions.processingFailure({ 
  ...
  download() {
    this.outputBlob$.subscribe(blob => {
      if (blob) {
        ...
        a.href = url;
        a.download = `audio_removed_video_${Date.now()}.mp4`;
        a.click();
  ...
  reset() {
    this.store.dispatch(AudioRemoverActions.resetState());
  }
}
