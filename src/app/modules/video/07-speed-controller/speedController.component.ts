  ...
  template: `
    <div class="p-8 glass-panel rounded-2xl max-w-4xl mx-auto">
      <h2 class="text-3xl font-bold mb-4 flex items-center gap-2">
        <mat-icon class="text-accent-cyan">speed</mat-icon> Speed Controller
      </h2>
      <p class="text-text-secondary mb-8">Adjust playback speed (Slo-mo or Time-lapse).</p>
      <div class="aspect-video bg-black/40 rounded-xl flex items-center justify-center border border-dashed border-white/20">
        <span class="text-text-muted">Select video to adjust speed</span>
      </div>
    </div>
  `
})
