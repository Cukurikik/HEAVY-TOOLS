  ...
  template: `
    <div class="p-8 max-w-4xl mx-auto space-y-8">
      <header class="space-y-2">
        ...
        <p class="text-text-secondary">Adjust video playback speed. Create slow-motion or time-lapse effects easily.</p>
      </header>
        ...
        @if ((status$ | async) === 'idle' || (status$ | async) === null || (status$ | async) === undefined) {
          ...
          <div class="flex flex-col items-center gap-6 w-full max-w-md">
            ...
            <p class="text-accent-cyan animate-pulse font-medium">
              {{ (status$ | async) === 'loading' ? 'Analyzing Video...' : 'Adjusting Speed...' }}
            </p>
          </div>
          ...
          <div class="flex flex-col items-center gap-6 text-center">
            ...
            <div>
              <p class="text-xl font-bold">Speed Adjustment Complete!</p>
      ...
      @if ((state$ | async)?.videoMeta; as meta) {
        <div class="glass-panel p-6 rounded-2xl border border-white/5 space-y-6">
          <div class="flex justify-between items-center">
            <h3 class="font-bold text-lg">Speed Settings</h3>
            <span class="text-accent-cyan font-mono text-sm">{{ (state$ | async)?.speed }}x</span>
          </div>
          
          <div class="grid grid-cols-4 gap-4">
            @for (s of [0.25, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0]; track s) {
              <button (click)="onSpeedSelect(s)" 
                      [class.bg-accent-cyan]="(state$ | async)?.speed === s"
                      [class.text-black]="(state$ | async)?.speed === s"
                      [class.bg-white/5]="(state$ | async)?.speed !== s"
                      class="py-2 rounded-lg font-mono transition-all duration-300 hover:bg-white/10">
                {{ s }}x
              </button>
            }
          </div>
