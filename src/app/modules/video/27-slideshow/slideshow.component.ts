  ...
  template: `
    <div class="min-h-screen bg-[#0a0a0f] p-6 space-y-6">
      <header class="space-y-1">
        ...
        <p class="text-white/50 text-sm">Create a video slideshow from a sequence of images with Ken Burns effect and music</p>
      </header>
...
export class SlideshowComponent implements OnDestroy {
  ...
  async onFileSelected(files: File[]) {
    const file = files[0];
    this.store.dispatch(SlideshowActions.loadFile({ file }));
    try {
      const meta = await this.ffmpeg.getMetadata(file);
      this.store.dispatch(SlideshowActions.loadMetaSuccess({ meta }));
    } catch {
      this.store.dispatch(SlideshowActions.loadMetaFailure({
