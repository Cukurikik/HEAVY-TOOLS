...
export class ToHtmlComponent implements OnDestroy {
  store = inject(Store);
  ...
  onProcess() {
     ...
     this.state$.subscribe(state => {
        ...
        this.bridge.process<any, Blob>(
          () => new Worker(new URL('./toHtml.worker', import.meta.url), { type: 'module' }),
          { file: state.inputFile }
        ).subscribe({
           next: (msg) => {
              if (msg.type === 'progress') this.store.dispatch(ToHtmlActions.updateProgress({ progress: msg.value ?? 0 }));
