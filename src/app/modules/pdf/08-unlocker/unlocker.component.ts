...
import { UnlockerActions, selectUnlockerPdfState, selectUnlockerIsLoading, selectUnlockerCanProcess } from './unlocker.store';
...
export class UnlockerComponent implements OnDestroy {
  store = inject(Store);
  ...
  onProcess() {
     ...
     this.state$.pipe(take(1)).subscribe(state => {
        ...
        this.bridge.process<unknown, Blob>(
          () => new Worker(new URL('./unlocker.worker', import.meta.url), { type: 'module' }),
          { file: state.inputFile }
        ).subscribe({
           next: (msg) => {
              if (msg.type === 'progress') this.store.dispatch(UnlockerActions.updateProgress({ progress: msg.value ?? 0 }));
