...
export class ToWordComponent implements OnDestroy {
  store = inject(Store);
  ...
  onProcess() {
     ...
     this.state$.subscribe(state => {
        ...
        this.bridge.process<any, Blob>(
          () => new Worker(new URL('./toWord.worker', import.meta.url), { type: 'module' }),
          { file: state.inputFile }
        ).subscribe({
           next: (msg) => {
              if (msg.type === 'progress') this.store.dispatch(ToWordActions.updateProgress({ progress: msg.value ?? 0 }));
