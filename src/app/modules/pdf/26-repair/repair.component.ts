...
import { RepairActions, selectRepairPdfState, selectRepairIsLoading, selectRepairCanProcess } from './repair.store';
...
export class RepairComponent implements OnDestroy {
  store = inject(Store);
  ...
  onProcess() {
     ...
     this.state$.subscribe(state => {
        ...
        this.bridge.process<unknown, Blob>(
          () => new Worker(new URL('./repair.worker', import.meta.url), { type: 'module' }),
          { file: state.inputFile }
        ).subscribe({
           next: (msg) => {
              if (msg.type === 'progress') this.store.dispatch(RepairActions.updateProgress({ progress: msg.value ?? 0 }));
