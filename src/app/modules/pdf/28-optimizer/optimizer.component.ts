...
import { OptimizerActions, selectOptimizerPdfState, selectOptimizerIsLoading, selectOptimizerCanProcess } from './optimizer.store';
...
export class OptimizerComponent implements OnDestroy {
  store = inject(Store);
  ...
  onProcess() {
     ...
     this.state$.subscribe(state => {
        ...
        this.bridge.process<unknown, Blob>(
          () => new Worker(new URL('./optimizer.worker', import.meta.url), { type: 'module' }),
          { file: state.inputFile }
        ).subscribe({
           next: (msg) => {
              if (msg.type === 'progress') this.store.dispatch(OptimizerActions.updateProgress({ progress: msg.value ?? 0 }));
