...
export class ImageExtractorComponent implements OnDestroy {
  store = inject(Store);
  ...
  onProcess() {
     ...
     this.state$.subscribe(state => {
        ...
        this.bridge.process<unknown, Blob>(
          () => new Worker(new URL('./imageExtractor.worker', import.meta.url), { type: 'module' }),
          { file: state.inputFile }
        ).subscribe({
           next: (msg) => {
              if (msg.type === 'progress') this.store.dispatch(ImageExtractorActions.updateProgress({ progress: msg.value ?? 0 }));
