...
import { RedactorActions, selectRedactorPdfState, selectRedactorIsLoading, selectRedactorCanProcess } from './redactor.store';
...
export class RedactorComponent implements OnDestroy {
  store = inject(Store);
  ...
  onProcess() {
     ...
     this.state$.subscribe(state => {
        ...
        this.bridge.process<unknown, Blob>(
          () => new Worker(new URL('./redactor.worker', import.meta.url), { type: 'module' }),
          { file: state.inputFile }
        ).subscribe({
           next: (msg) => {
              if (msg.type === 'progress') this.store.dispatch(RedactorActions.updateProgress({ progress: msg.value ?? 0 }));
