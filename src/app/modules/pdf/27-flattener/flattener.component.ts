...
import { FlattenerActions, selectFlattenerPdfState, selectFlattenerIsLoading, selectFlattenerCanProcess } from './flattener.store';
...
export class FlattenerComponent implements OnDestroy {
  store = inject(Store);
  ...
  onProcess() {
     ...
     this.state$.pipe(take(1)).subscribe(state => {
        ...
        this.bridge.process<unknown, Blob>(
          () => new Worker(new URL('./flattener.worker', import.meta.url), { type: 'module' }),
          { file: state.inputFile }
        ).subscribe({
           next: (msg) => {
              if (msg.type === 'progress') this.store.dispatch(FlattenerActions.updateProgress({ progress: msg.value ?? 0 }));
