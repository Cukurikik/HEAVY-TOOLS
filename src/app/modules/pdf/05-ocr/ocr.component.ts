...
import { OcrActions, selectOcrPdfState, selectOcrIsLoading, selectOcrCanProcess } from './ocr.store';
  ...
  onProcess() {
     ...
     this.state$.pipe(take(1)).subscribe(state => {
        ...
        this.bridge.process<unknown, Blob>(
          () => new Worker(new URL('./ocr.worker', import.meta.url), { type: 'module' }),
          { file: state.inputFile }
        ).subscribe({
           next: (msg) => {
              if (msg.type === 'progress') this.store.dispatch(OcrActions.updateProgress({ progress: msg.value ?? 0 }));
