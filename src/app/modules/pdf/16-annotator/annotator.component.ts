...
import { AnnotatorActions, selectAnnotatorPdfState, selectAnnotatorIsLoading, selectAnnotatorCanProcess } from './annotator.store';
...
export class AnnotatorComponent implements OnDestroy {
  store = inject(Store);
     ...
     this.state$.pipe(take(1)).subscribe(state => {
        ...
        this.bridge.process<unknown, Blob>(
          ...
          { file: state.inputFile }
           ...
           next: (msg) => {
              if (msg.type === 'progress') this.store.dispatch(AnnotatorActions.updateProgress({ progress: msg.value ?? 0 }));
