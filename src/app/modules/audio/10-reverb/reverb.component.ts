...
import { reverbFeature, reverbActions } from './reverb.store';
      ...
      <div class="max-w-4xl mx-auto space-y-6">
        ...
        @if ((state$ | async)?.inputFile) {
          <div class="bg-[#12121a] rounded-2xl p-6 border border-white/5 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">@for(ctrl of ['Room Size','Decay','Pre-Delay','Wet/Dry','Damping'];track ctrl){<div><span class="text-xs text-white/40" style="display: block;">{{ctrl}}</span><input type="range" class="w-full accent-purple-400"></div>}</div>
          </div>
