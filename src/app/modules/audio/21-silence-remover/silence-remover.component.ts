...
import { silenceRemoverFeature, silenceRemoverActions } from './silence-remover.store';
      ...
      <div class="max-w-4xl mx-auto space-y-6">
        ...
        @if ((state$ | async)?.inputFile) {
          <div class="bg-[#12121a] rounded-2xl p-6 border border-white/5 space-y-4">
            ...
            <div><span class="text-xs text-white/40" style="display: block;">Threshold (dBFS)</span><input type="range" class="w-full accent-amber-400" min="-60" max="-10" [(ngModel)]="localThreshold"><p class="text-center text-sm text-white/50">{{localThreshold}} dB</p></div>
          </div>
