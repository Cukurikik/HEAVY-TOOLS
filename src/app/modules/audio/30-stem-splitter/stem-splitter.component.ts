...
import { StemSplitterState, stemSplitterFeature, stemSplitterActions } from './stem-splitter.store';
    ...
    <div class="max-w-7xl mx-auto space-y-8 animate-fade-in-up">
      ...
      <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 drop-shadow-lg tracking-tight" class="text-xl font-bold text-white">AI Stem Splitter</h1>
        ...
        @if ((state$ | async)?.inputFile) {
          <div class="bg-[#12121a] rounded-2xl p-6 border border-white/5 space-y-4">
            ...
            <div class="flex gap-2 mb-4">@for(m of [['demucs-v4','Demucs v4 (Best)'],['htdemucs','HTDemucs (Fast)'],['spleeter-2stem','Spleeter 2-Stem']];track m[0]){<button class="px-3 py-1.5 rounded-lg text-xs" [class.bg-purple-500]="localStemModel===m[0]" (click)="localStemModel=m[0]">{{m[1]}}</button>}</div><div class="grid grid-cols-4 gap-2 mt-4">@for(stem of [['🎤','Vocals'],['🥁','Drums'],['🎸','Bass'],['🎹','Other']];track stem[1]){<div class="p-3 rounded-xl bg-white/5 text-center"><span class="text-2xl">{{stem[0]}}</span><p class="text-[10px] text-white/40 mt-1">{{stem[1]}}</p></div>}</div>
          </div>
