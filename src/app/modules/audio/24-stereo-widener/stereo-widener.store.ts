...
export const stereoWidenerFeature = createFeature({
  name: 'stereoWidener',
  reducer: createReducer(
    initialState,
    on(stereoWidenerActions.loadFile, (s, { file }) => ({ ...s, inputFile: file, status: 'loading' as const, errorCode: null, errorMessage: null })),
    on(stereoWidenerActions.loadFileSuccess, (s, { meta, waveformData }) => ({ ...s, audioMeta: meta, waveformData, status: 'idle' as const })),
    on(stereoWidenerActions.loadFileFailure, (s, { errorCode, message }) => ({ ...s, status: 'error' as const, errorCode, errorMessage: message, retryable: true })),
    on(stereoWidenerActions.startProcessing, s => ({ ...s, status: 'processing' as const, progress: 0, errorCode: null, errorMessage: null })),
...
// ── Effects ──
@Injectable()
export class StereoWidenerEffects {
  private actions$ = inject(Actions);
  private store = inject(Store);
  private svc = inject(StereoWidenerService);
  private bridge = inject(AudioWorkerBridgeService);

  loadFile$ = createEffect(() => this.actions$.pipe(
    ofType(stereoWidenerActions.loadFile),
    switchMap(({ file }) => from(this.svc.decodeAndAnalyze(file)).pipe(
      map(({ meta, waveformData }) => stereoWidenerActions.loadFileSuccess({ meta, waveformData })),
      catchError(err => of(stereoWidenerActions.loadFileFailure({ errorCode: 'DECODE_FAILED', message: err?.message ?? 'Decode failed' })))
    ))
  ));
  ...
  download$ = createEffect(() => this.actions$.pipe(
    ofType(stereoWidenerActions.downloadOutput),
    withLatestFrom(this.store.select(stereoWidenerFeature.selectStereoWidenerState)),
    tap(([, state]) => {
