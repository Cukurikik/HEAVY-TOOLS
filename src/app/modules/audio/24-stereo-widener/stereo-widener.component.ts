...
import { stereoWidenerFeature, stereoWidenerActions } from './stereo-widener.store';
      ...
      <div class="max-w-4xl mx-auto space-y-6">
        ...
        @if ((state$ | async)?.inputFile) {
          <div class="bg-[#12121a] rounded-2xl p-6 border border-white/5 space-y-4">
            ...
            <div><span class="text-xs text-white/40" style="display: block;">Width (0% = Mono, 100% = Normal, 200% = Extra Wide)</span><input type="range" class="w-full accent-violet-400" min="0" max="200" [(ngModel)]="localWidth"><p class="text-center text-lg font-bold text-violet-400">{{localWidth}}%</p></div>
          </div>
