...
import {
  dynamicsCompressorFeature,
  dynamicsCompressorActions,
    ...
    <div class="min-h-screen bg-[#0a0a0f] p-6">
      <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4">
          <div
            class="w-12 h-12 rounded-xl bg-gradient-to-br from-cyan-500/20 to-purple-500/20 flex items-center justify-center border border-white/10"
          >
        ...
        @if ((state$ | async)?.inputFile) {
          <div
            class="bg-[#12121a] rounded-2xl p-6 border border-white/5 space-y-4"
          >
            <app-waveform-display
              [waveformData]="(state$ | async)?.waveformData ?? null"
            ></app-waveform-display>
            <div class="grid grid-cols-2 gap-4">
              @for (
                ...
                <div>
                  <span class="text-xs text-white/40" style="display: block;">{{
                    ctrl[0]
                  }}</span>
                  <p class="text-xs text-white/20">{{ ctrl[1] }}</p>
                  <input type="range" class="w-full accent-cyan-400" />
                </div>
              }
            </div>
          </div>
          ...
          <div class="flex items-center gap-4">
            @if (
              (state$ | async)?.status === "processing" ||
              (state$ | async)?.status === "rendering"
            ) {
              <app-audio-progress-ring
                [progress]="(state$ | async)?.progress ?? 0"
              ></app-audio-progress-ring>
              <span class="text-sm text-white/50">Processing...</span>
            } @else {
              <button
                class="flex-1 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-purple-500 text-white font-bold text-sm hover:opacity-90 transition-opacity"
                [disabled]="(state$ | async)?.status !== 'idle'"
                (click)="onProcess()"
              >
                ⚡ Process
              </button>
          ...
          @if ((state$ | async)?.status === "error") {
            ...
            >
              ...
              @if ((state$ | async)?.retryable) {
                <button class="ml-3 underline" (click)="onProcess()">
                  Retry
                </button>
              }
            </div>
          ...
          @if ((state$ | async)?.outputBlob) {
            <app-audio-player
              [audioBlob]="(state$ | async)?.outputBlob ?? null"
            ></app-audio-player>
            <app-audio-export-panel
              [disabled]="!(state$ | async)?.outputBlob"
...
export class DynamicsCompressorComponent implements OnDestroy {
  ...
  state$ = this.store.select(
    dynamicsCompressorFeature.selectDynamicsCompressorState,
  );
