...
import { NoiseRemoverState, noiseRemoverFeature, noiseRemoverActions } from './noise-remover.store';
    ...
    <div class="max-w-7xl mx-auto space-y-8 animate-fade-in-up">
      ...
      <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 drop-shadow-lg tracking-tight" class="text-xl font-bold text-white">Noise Remover</h1>
        ...
        @if ((state$ | async)?.inputFile) {
          <div class="bg-[#12121a] rounded-2xl p-6 border border-white/5 space-y-4">
            ...
            <div class="flex gap-2 mb-4">@for(a of ['spectral','ai','ffmpeg'];track a){<button class="relative flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-3 rounded-xl font-bold text-sm tracking-wide text-white bg-gradient-to-r from-blue-600 to-indigo-600 shadow-[0_0_20px_rgba(79,70,229,0.3)] hover:shadow-[0_0_30px_rgba(79,70,229,0.5)] transition-all duration-300 transform hover:-translate-y-1 group disabled:opacity-50 disabled:cursor-not-allowed overflow-hidden" [class.bg-cyan-500]="localAlgo===a" (click)="localAlgo=a">
        <div class="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
        <span class="relative z-10 flex items-center gap-2">{{a==='ai'?'AI (RNNoise)':a==='spectral'?'Spectral':'FFmpeg'}}</span>
      </button>}</div><div><label class="text-xs text-white/40">Strength</label><input type="range" class="w-full accent-cyan-400" min="0" max="100" [(ngModel)]="localStrength"></div>
          </div>
