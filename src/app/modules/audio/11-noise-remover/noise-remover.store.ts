...
export const noiseRemoverFeature = createFeature({
  name: 'noiseRemover',
  reducer: createReducer(
    initialState,
    on(noiseRemoverActions.loadFile, (s, { file }) => ({ ...s, inputFile: file, status: 'loading' as const, errorCode: null, errorMessage: null })),
    on(noiseRemoverActions.loadFileSuccess, (s, { meta, waveformData }) => ({ ...s, audioMeta: meta, waveformData, status: 'idle' as const })),
    on(noiseRemoverActions.loadFileFailure, (s, { errorCode, message }) => ({ ...s, status: 'error' as const, errorCode, errorMessage: message, retryable: true })),
    on(noiseRemoverActions.startProcessing, s => ({ ...s, status: 'processing' as const, progress: 0, errorCode: null, errorMessage: null })),
...
// ── Effects ──
@Injectable()
export class NoiseRemoverEffects {
  private actions$ = inject(Actions);
  private store = inject(Store);
  private svc = inject(NoiseRemoverService);
  private bridge = inject(AudioWorkerBridgeService);

  loadFile$ = createEffect(() => this.actions$.pipe(
    ofType(noiseRemoverActions.loadFile),
    switchMap(({ file }) => from(this.svc.decodeAndAnalyze(file)).pipe(
      map(({ meta, waveformData }) => noiseRemoverActions.loadFileSuccess({ meta, waveformData })),
      catchError(err => of(noiseRemoverActions.loadFileFailure({ errorCode: 'DECODE_FAILED', message: err?.message ?? 'Decode failed' })))
    ))
  ));

  startProcessing$ = createEffect(() => this.actions$.pipe(
    ofType(noiseRemoverActions.startProcessing),
    withLatestFrom(this.store.select(noiseRemoverFeature.selectNoiseRemoverState)),
    exhaustMap(([, state]) => {
      if (!state.inputFile) return of(noiseRemoverActions.processingFailure({ errorCode: 'INVALID_PARAMS', message: 'No input file', retryable: true }));
      return from(this.svc.processAudio(state.inputFile)).pipe(
        map(blob => noiseRemoverActions.processingSuccess({ outputBlob: blob, outputSizeMB: blob.size / 1048576 })),
        catchError(err => of(noiseRemoverActions.processingFailure({ errorCode: 'WORKER_CRASHED', message: err?.message ?? 'Processing failed', retryable: true })))
      );
    })
  ));

  download$ = createEffect(() => this.actions$.pipe(
    ofType(noiseRemoverActions.downloadOutput),
    withLatestFrom(this.store.select(noiseRemoverFeature.selectNoiseRemoverState)),
    tap(([, state]) => {
