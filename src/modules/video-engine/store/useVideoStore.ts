...
// ═══════════════════════════════════════════════════
// Helper: build FFmpeg args per operation
// ═══════════════════════════════════════════════════

interface FFmpegBuildResult {
  args: string[];
  outputName: string;
...
async function buildFFmpegArgs(
  ...
  switch (task.operation) {
    // 1. VIDEO TRIMMER — FFmpeg -ss -i -t libx264
    case "trimmer": {
      const start = (opts.start as string) || "00:00:00";
      const end = (opts.end as string) || "00:00:10";
      args = ["-i", inputName, "-ss", start, "-to", end, "-c:v", "libx264", "-c:a", "aac", outputName];
      break;
    }

    // 2. VIDEO MERGER — concat protocol list.txt
    case "merger": {
      const filesToMerge = task.files.length > 0 ? task.files : [task.file].filter(Boolean) as File[];
      let concatList = "";
    ...
    // 3. VIDEO CONVERTER — -c:v libx264/vp9/av1
    case "converter": {
      ...
      if (format === "webm") {
        ...
        args = ["-i", inputName, "-c:v", "libx264", "-preset", "fast", "-c:a", "aac", outputName];
      }
      break;
    }

    // 4. VIDEO COMPRESSOR — -crf -b:v -maxrate
    case "compressor": {
      const crf = (opts.crf as number) || 28;
      const preset = (opts.preset as string) || "medium";
      args = ["-i", inputName, "-c:v", "libx264", "-crf", crf.toString(), "-preset", preset, "-c:a", "aac", outputName];
      break;
    }

    // 5. VIDEO FLIPPER — -vf hflip / vflip
    case "flipper": {
    ...
    // 6. VIDEO ROTATOR — -vf transpose libx264
    case "rotator": {
    ...
    // 7. VIDEO STABILIZER — server-side (deshake fallback)
    case "stabilizer": {
      args = ["-i", inputName, "-vf", "deshake", "-c:a", "copy", outputName];
      break;
    }

    // 8. VIDEO REVERSER — -vf reverse -af areverse
    case "reverse": {
      args = ["-i", inputName, "-vf", "reverse", "-af", "areverse", outputName];
      break;
    }

    // 9. SPEED CONTROLLER — -vf setpts -af atempo
    case "speed-control": {
    ...
    // 10. LOOP ENGINE — -stream_loop -N concat
    case "loop-engine": {
    ...
    // 11. PRO EDITOR — full CRF + bitrate control
    case "pro-editor": {
    ...
    // 12. THUMBNAIL EXTRACTOR — -ss -frames:v 1 Canvas API
    case "thumbnail-extractor": {
      const timestamp = (opts.timestamp as string) || "00:00:01";
      outputName = "thumbnail.png";
      outputMimeType = "image/png";
      args = ["-i", inputName, "-ss", timestamp, "-frames:v", "1", "-q:v", "2", outputName];
      break;
    }

    // 13. SUBTITLE BURNER — -vf subtitles= / ass=
    ...
    // 16. COLOR GRADER — -vf eq= curves= colorbalance=
    case "color-grader": {
    ...
    // 17. AI UPSCALER — bicubic fallback (real AI in worker)
    ...
    // 18. FRAME INTERPOLATOR — -vf minterpolate=fps=
    ...
    // 19. GIF CONVERTER — palettegen + paletteuse
    case "gif-converter": {
    ...
    // 20. HDR TONEMAPPER — zscale + tonemap hable
    case "hdr-tonemapper": {
    ...
    // 21. BLACK & WHITE — -vf hue=s=0
    case "black-white": {
      args = ["-i", inputName, "-vf", "hue=s=0", "-c:a", "copy", outputName];
      break;
    }

    // 22. SLOW MOTION — setpts + minterpolate
    case "slow-motion": {
      const slowFactor = (opts.factor as number) || 0.5;
      const slowPts = (1 / slowFactor).toFixed(4);
      args = ["-i", inputName, "-vf", `setpts=${slowPts}*PTS,minterpolate=fps=30:mi_mode=mci`, "-an", outputName];
      break;
    }

    // 23. TIMELAPSE MAKER — -vf framestep= setpts=
    case "timelapse": {
      const tlSpeed = (opts.speed as number) || 10;
      const tlStep = Math.max(1, Math.round(tlSpeed));
      args = ["-i", inputName, "-vf", `framestep=${tlStep},setpts=N/30/TB`, "-an", outputName];
      break;
    }

    // 24. SCREEN RECORDER — handled separately
    case "screen-recorder": {
      break;
    }

    // 25. METADATA EDITOR — -metadata key=value
    ...
    // 26. BATCH PROCESSOR — FFmpeg + InMemoryQueue
    case "batch-processor": {
      const batchOp = (opts.batchOperation as string) || "compress";
      const batchCrf = (opts.batchCrf as number) || 28;
      if (batchOp === "compress") {
        args = ["-i", inputName, "-c:v", "libx264", "-crf", batchCrf.toString(), "-preset", "fast", "-c:a", "aac", outputName];
    ...
    // 27. CHAPTER MARKER — -i -map_metadata chapters
    case "chapter-marker": {
    ...
    // 28. AUDIO EXTRACTOR — -vn -acodec copy
    case "audio-extractor": {
      ...
      if (audioFmt === "mp3") {
        ...
        args = ["-i", inputName, "-vn", "-c:a", "aac", outputName];
      }
      break;
    }

    // 29. VIDEO SPLITTER — -ss -t segment_time
    case "video-splitter": {
    ...
    // 30. ASPECT RATIO TOOL — -vf scale + pad / crop
    case "aspect-ratio": {
  ...
  return { args, outputName, outputMimeType };
}
...
export const useVideoStore = create<VideoStore>((set, get) => ({
  ...
  processVideo: async () => {
    ...
    try {
      ...
      // Build FFmpeg args for the current operation
      const { args, outputName, outputMimeType } = await buildFFmpegArgs(task, inputName, ffmpeg);

      // Attach log listener
      const logCallback = ({ message }: { message: string }) => {
        set((state) => ({
          ffmpegLogs: [...state.ffmpegLogs, message],
        }));
      };
      ...
      if (args.length > 0) {
        ...
        if (ret !== 0) {
          const { ffmpegLogs } = get();
          console.error("FFmpeg Video Logs:", ffmpegLogs.join("\n"));
          throw new Error(`FFmpeg failed with code ${ret}. Check logs for details.`);
        }
      } else {
        ffmpeg.off("log", logCallback);
      }
