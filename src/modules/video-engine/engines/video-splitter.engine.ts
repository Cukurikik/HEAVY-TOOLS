...
export async function buildVideoSplitterArgs(input: string, output: string, opts: VideoSplitterOptions, ffmpeg?: FFmpeg, files?: File[]): Promise<string[]> {
  ...
  // We use a hardcoded segment pattern, but we return the exact name of the first chunk to the store so it doesn't crash reading a wildcard
  return ["-i", input, "-c", "copy", "-f", "segment", "-segment_time", segDuration.toString(), "-reset_timestamps", "1", "segment_%03d.mp4"];
}
export function getVideoSplitterOutputName(opts: VideoSplitterOptions): string { return "segment_000.mp4"; }
